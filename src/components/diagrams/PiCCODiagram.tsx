...
const PiCCODiagram = () => {
  ...
  const piccoParams = [
    { id: "ci", name: "CI", full: "Cardiac Index", normal: "3.0–5.0 L/min/m²", unit: "L/min/m²", category: "Flow", measurement: "Measured by transpulmonary thermodilution (TPTD): cold saline injected via CVC, temperature change detected at femoral artery thermistor. CO calculated using Stewart-Hamilton equation: CO = (Vᵢ × (Tᵦ − Tᵢ) × K) / ∫ΔT·dt. CI = CO / BSA. Between calibrations, continuously estimated by pulse contour analysis — SV derived from area under systolic arterial waveform, multiplied by HR.", interpretation: "Low CI (<3.0): cardiogenic shock, hypovolaemia, or excessive afterload. High CI (>5.0): hyperdynamic state — sepsis, liver failure, thyrotoxicosis. Interpret with SVR and preload.", color: "hsl(var(--primary))" },
    { id: "gedi", name: "GEDI", full: "Global End-Diastolic Volume Index", normal: "680–800 mL/m²", unit: "mL/m²", category: "Preload", measurement: "Derived from TPTD using mean transit time (MTt) and exponential downslope time (DSt). ITTV (intrathoracic thermal volume) = CO × MTt. PTV (pulmonary thermal volume) = CO × DSt. GEDV = ITTV − PTV. Represents combined end-diastolic volume of all four cardiac chambers. GEDI = GEDV / BSA. Requires thermodilution calibration — not available from pulse contour alone.", interpretation: "Volumetric preload marker — superior to CVP/PAOP. Low GEDI (<680): hypovolaemia — likely fluid responsive. High GEDI (>800): volume overloaded or cardiac dysfunction.", color: "hsl(200 70% 50%)" },
    { id: "evlwi", name: "EVLWI", full: "Extravascular Lung Water Index", normal: "3–7 mL/kg", unit: "mL/kg", category: "Lung water", measurement: "Derived from TPTD. EVLW = ITTV − ITBV, where ITBV (intrathoracic blood volume) = 1.25 × GEDV (empirical relationship). ITTV = CO × MTt. EVLW represents water in the lung interstitium and alveoli (not intravascular). EVLWI = EVLW / predicted body weight (PBW, not actual weight). Only bedside monitor that quantifies lung water directly — validated against gravimetric post-mortem measurements.", interpretation: "Quantifies pulmonary oedema. 7–10: mild oedema. 10–14: moderate. >14: severe (ARDS). Guides fluid restriction and diuresis. Prognostic in ARDS (↑EVLWI = ↑mortality).", color: "hsl(142 60% 45%)" },
    { id: "svri", name: "SVRI", full: "Systemic Vascular Resistance Index", normal: "1700–2400 dyn·s/cm⁵·m²", unit: "dyn·s/cm⁵·m²", category: "Afterload", measurement: "Calculated (not directly measured): SVRI = 80 × (MAP − CVP) / CI. Derived from continuously available pulse contour CI and invasive arterial MAP. CVP input manually or from CVC transducer. Updated beat-to-beat. Represents arteriolar resistance (afterload). Units: dyn·s/cm⁵·m² (multiply by 80 to convert from Wood units).", interpretation: "Low SVRI (<1700): vasodilation — sepsis, anaphylaxis, neurogenic shock, liver failure. High SVRI (>2400): vasoconstriction — cardiogenic shock (compensatory), hypovolaemia, vasopressor excess.", color: "hsl(25 80% 50%)" },
    { id: "gef", name: "GEF", full: "Global Ejection Fraction", normal: "25–35%", unit: "%", category: "Contractility", measurement: "Calculated from thermodilution-derived values: GEF = 4 × SV / GEDV. SV = CO / HR. Represents the ratio of stroke volume to total cardiac filling volume. Called 'global' because it reflects biventricular function (not just LV like echocardiographic LVEF). Normal GEF (25–35%) is lower than LVEF (55–70%) because GEDV includes all four chambers. Requires thermodilution calibration.", interpretation: "Estimates cardiac contractility. Low GEF (<25%): myocardial dysfunction — consider inotropes (dobutamine, levosimendan). Not the same as LVEF on echo. Better than dP/dt for contractility assessment.", color: "hsl(0 70% 50%)" },
    { id: "svv", name: "SVV", full: "Stroke Volume Variation", normal: "<10%", unit: "%", category: "Fluid responsiveness", measurement: "Continuously derived from pulse contour analysis (no thermodilution needed). SVV = (SVmax − SVmin) / SVmean × 100, calculated over a 30-second sliding window. SV estimated beat-to-beat from area under the systolic portion of the arterial pressure waveform, calibrated by the most recent thermodilution. Reflects cyclic preload changes caused by positive pressure ventilation — heart-lung interaction.", interpretation: "Dynamic preload marker. SVV >12%: likely fluid responsive (on steep Starling curve). Requirements: MV, VT ≥8 mL/kg, sinus rhythm, closed chest. Continuously displayed.", color: "hsl(45 80% 50%)" },
    { id: "ppv", name: "PPV", full: "Pulse Pressure Variation", normal: "<13%", unit: "%", category: "Fluid responsiveness", measurement: "Continuously derived from the arterial waveform (no thermodilution needed). PPV = (PPmax − PPmin) / PPmean × 100 over one respiratory cycle. PP = systolic − diastolic pressure, measured beat-to-beat. Calculated over a 30-second window. Same physiological basis as SVV — positive pressure ventilation cyclically alters LV preload via changes in venous return and RV/LV interdependence.", interpretation: "PPV >13%: fluid responsive. Same requirements as SVV. Slightly better specificity than SVV in some studies. Both continuously updated — no recalibration needed.", color: "hsl(45 80% 50%)" },
    { id: "pvpi", name: "PVPI", full: "Pulmonary Vascular Permeability Index", normal: "1.0–3.0", unit: "ratio", category: "Lung water", measurement: "Calculated from thermodilution-derived values: PVPI = EVLW / PBV, where PBV (pulmonary blood volume) = ITBV − GEDV. ITBV = 1.25 × GEDV. Dimensionless ratio. Reflects the permeability of the alveolar-capillary membrane. A high PVPI with elevated EVLW indicates increased permeability (ARDS), while a low PVPI with elevated EVLW indicates hydrostatic oedema (heart failure). Unique to PiCCO.", interpretation: "PVPI 1–3 + ↑EVLWI = cardiogenic oedema. PVPI >3 + ↑EVLWI = permeability oedema (ARDS). Distinguishes aetiology — cannot be obtained from PAC.", color: "hsl(142 60% 45%)" },
  ];
