...
import { casesForTopic } from "@/data/perioperativeCaseIndex";
...
export const RelatedCases = ({ topicId }: RelatedCasesProps) => {
  const relatedCases = casesForTopic(topicId);
    ...
    <section id="related-cases" className="scroll-mt-24 border-y border-border py-6">
      <div className="flex items-start gap-3 mb-4">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Apply this topic in realistic, progressive perioperative scenarios.</p>
        </div>
      </div>

      <div className="grid gap-3 sm:grid-cols-2">
        {relatedCases.map((caseItem) => (
          <article key={caseItem.id} className="border border-border bg-card rounded-lg p-4 min-w-0">
            ...
            <Button asChild variant="link" size="sm" className="mt-2 h-auto p-0 whitespace-normal text-left justify-start">
              <Link to={`/perioperative/case-bank#${caseItem.id}`}>
