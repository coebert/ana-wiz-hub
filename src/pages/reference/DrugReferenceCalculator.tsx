...
export default function DrugReferenceCalculator() {
  const [params] = useSearchParams();
  const prefillDrug = params.get("drug");
  const prefillWeight = Number(params.get("weight"));
      ...
      <main className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        {dilution.slug && (
          <p className="mt-6 text-sm">
            <Link to={`/reference/drugs/${dilution.slug}`} className="text-primary underline">
