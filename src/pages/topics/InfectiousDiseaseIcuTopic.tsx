...
import InfectionSiteMapDiagram from "@/components/diagrams/intensive-care/InfectionSiteMapDiagram";
import HAIPathogenesisCascadeDiagram from "@/components/diagrams/intensive-care/HAIPathogenesisCascadeDiagram";
import PCPHRCTDiagram from "@/components/diagrams/intensive-care/PCPHRCTDiagram";
import PCPManagementFlowchart from "@/components/diagrams/intensive-care/PCPManagementFlowchart";
  ...
  return (
    <TopicTemplate
      ...
      sectionSources={{
        ...
        workedExamples: ["IDSA HAP/VAP 2016", "NICE NG191", "SSC 2021"],
      }}
      keyPoints={[
      ...
      coreConcepts={
        ...
        <>
          ...
          <ExamSection id="respiratory" exams={[Exam.FINAL, Exam.FFICM, Exam.EDIC]} curriculumCodes={["FFICM 4.7"]}>
            <CollapsibleSubsection title="Respiratory Infections" defaultOpen>
            <p className="text-muted-foreground leading-relaxed mb-3">
          ...
          <ExamSection id="legionella" exams={[Exam.FINAL, Exam.FFICM, Exam.EDIC]}>
            <CollapsibleSubsection title="Legionella Pneumonia">
            <p className="text-muted-foreground leading-relaxed mb-3">
          ...
          <ExamSection id="sepsis" exams={[Exam.FINAL, Exam.FFICM, Exam.EDIC]}>
            <CollapsibleSubsection title="Gram-Positive vs Gram-Negative Sepsis">
            <p className="text-muted-foreground leading-relaxed mb-3">
          ...
          <ExamSection id="cns" exams={[Exam.FINAL, Exam.FFICM, Exam.EDIC]}>
            <CollapsibleSubsection title="CNS Infections">
            <p className="text-muted-foreground leading-relaxed mb-3">
          ...
          <ExamSection id="bloodstream" exams={[Exam.FINAL, Exam.FFICM, Exam.EDIC]}>
            <CollapsibleSubsection title="Bloodstream & Line Infections">
            <p className="text-muted-foreground leading-relaxed mb-3">
          ...
          <ExamSection id="fungal" exams={[Exam.FINAL, Exam.FFICM, Exam.EDIC]}>
            <CollapsibleSubsection title="Fungal Infections in ICU">
            <p className="text-muted-foreground leading-relaxed mb-3">
            ...
            <h4 className="text-base font-serif font-semibold text-foreground mt-4 mb-2">Adjunctive treatment mini-pathways</h4>
            <p className="text-xs text-muted-foreground mb-2">
              Empirical pathways to run <em>in parallel</em> with co-trimoxazole when the corresponding co-pathogen is suspected. Each row carries an evidence-confidence chip (guideline GRADE) and a superscript ref linking to the source. Chip key: <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-accent/15 text-accent border border-accent/30 align-middle ml-1">Strong / High</span> <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-primary/10 text-primary border border-primary/30 align-middle ml-1">Conditional / Moderate</span> <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-muted text-muted-foreground border border-border align-middle ml-1">Expert opinion / Low</span>
            </p>
            <div className="grid md:grid-cols-2 gap-3 mb-4">
              ...
              <div className="rounded-lg border border-border bg-card p-3 text-xs space-y-2">
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  <p className="font-semibold text-foreground text-sm">CMV pneumonitis</p>
                  <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-accent/15 text-accent border border-accent/30">Strong (ECIL-7 / AST 2019)</span>
                </div>
                <p className="text-muted-foreground"><strong>Start when:</strong> BAL CMV PCR positive <em>and</em> compatible CT (diffuse GGO, nodules) <em>and</em> immunosuppressed host (HSCT, SOT, advanced HIV with CD4 &lt;100, biologics)<sup><a href="#ref-25" className="text-primary hover:underline">25</a>,<a href="#ref-26" className="text-primary hover:underline">26</a></sup> — or rising blood CMV viral load &gt;1000 IU/mL with hypoxia despite anti-PCP therapy.<sup><a href="#ref-25" className="text-primary hover:underline">25</a></sup> BAL PCR alone may reflect shedding — clinical-radiological correlation essential. <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-primary/10 text-primary border border-primary/30">Conditional</span></p>
                <p className="text-muted-foreground"><strong>1st-line:</strong> IV <strong>ganciclovir 5 mg/kg BD</strong> (induction 14–21 d, then maintenance 5 mg/kg OD); switch to oral <strong>valganciclovir 900 mg BD</strong> when stable.<sup><a href="#ref-25" className="text-primary hover:underline">25</a>,<a href="#ref-26" className="text-primary hover:underline">26</a></sup> <strong>Foscarnet 60 mg/kg TDS</strong> if myelosuppression or UL97 ganciclovir resistance.<sup><a href="#ref-25" className="text-primary hover:underline">25</a></sup> CMV hyperimmune globulin in HSCT pneumonitis. <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-accent/15 text-accent border border-accent/30">Strong</span></p>
                <p className="text-muted-foreground"><strong>Monitor:</strong> blood CMV PCR twice weekly (target ≥1 log drop by day 7, undetectable by day 14–21);<sup><a href="#ref-25" className="text-primary hover:underline">25</a></sup> FBC for neutropenia (pause/reduce ganciclovir if ANC &lt;0.5); U&amp;E + ionised Ca²⁺/Mg²⁺ daily on foscarnet; LFTs weekly. <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-accent/15 text-accent border border-accent/30">Strong</span></p>
                <p className="text-muted-foreground"><strong>Stop / step-down:</strong> after ≥14 d induction <em>and</em> two consecutive undetectable blood PCRs (≥1 wk apart) <em>and</em> clinical/radiological resolution.<sup><a href="#ref-25" className="text-primary hover:underline">25</a>,<a href="#ref-26" className="text-primary hover:underline">26</a></sup> Continue secondary prophylaxis (valganciclovir 900 mg OD) until immune reconstitution. Stop early if BAL PCR negative on repeat <em>and</em> alternative cause confirmed. <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-primary/10 text-primary border border-primary/30">Conditional</span></p>
              </div>

              {/* Invasive aspergillosis */}
              <div className="rounded-lg border border-border bg-card p-3 text-xs space-y-2">
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  <p className="font-semibold text-foreground text-sm">Invasive pulmonary aspergillosis (IPA)</p>
                  <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-accent/15 text-accent border border-accent/30">Strong (IDSA 2016 / ESCMID 2018)</span>
                </div>
                <p className="text-muted-foreground"><strong>Start when:</strong> probable/proven IPA by EORTC/MSGERC 2020 (host factor + clinical feature + mycological evidence): serum GM ≥0.5 OD <em>or</em> BAL GM ≥1.0 OD, BAL <em>Aspergillus</em> PCR positive, or positive culture/histology.<sup><a href="#ref-27" className="text-primary hover:underline">27</a>,<a href="#ref-28" className="text-primary hover:underline">28</a></sup> CAPA / IAPA in severe COVID/influenza ICU patients.<sup><a href="#ref-29" className="text-primary hover:underline">29</a></sup> <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-accent/15 text-accent border border-accent/30">Strong</span></p>
                <p className="text-muted-foreground"><strong>1st-line:</strong> IV <strong>voriconazole 6 mg/kg BD ×2 doses load → 4 mg/kg BD</strong> (or <strong>isavuconazole 200 mg TDS ×6 → 200 mg OD</strong> — preferred if QTc long, hepatic dysfunction or renal failure).<sup><a href="#ref-27" className="text-primary hover:underline">27</a>,<a href="#ref-28" className="text-primary hover:underline">28</a></sup> Reserve <strong>liposomal amphotericin B 3 mg/kg/day</strong> for azole intolerance / azole-resistant <em>A. fumigatus</em>. Combination (voriconazole + echinocandin) only as salvage. <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-accent/15 text-accent border border-accent/30">Strong</span></p>
                <p className="text-muted-foreground"><strong>Monitor:</strong> voriconazole TDS — trough <strong>1–5.5 mg/L</strong> at day 5 and after each dose change;<sup><a href="#ref-27" className="text-primary hover:underline">27</a></sup> QTc, LFTs twice-weekly; vision; CYP interactions (avoid rifampicin, phenytoin). Twice-weekly serum GM as response marker (≥1 log fall by day 14 = good response). Repeat HRCT at 7–14 d. <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-accent/15 text-accent border border-accent/30">Strong</span></p>
                <p className="text-muted-foreground"><strong>Stop / step-down:</strong> minimum <strong>6–12 weeks</strong>; continue until clinical + radiological resolution <em>and</em> reversal of immunosuppression.<sup><a href="#ref-27" className="text-primary hover:underline">27</a>,<a href="#ref-28" className="text-primary hover:underline">28</a></sup> Step down to oral azole when stable. Stop if mycology repeatedly negative <em>and</em> alternative diagnosis established <em>and</em> immune status restored. <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-primary/10 text-primary border border-primary/30">Conditional</span></p>
              </div>

              {/* TB */}
              <div className="rounded-lg border border-border bg-card p-3 text-xs space-y-2">
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  <p className="font-semibold text-foreground text-sm">Pulmonary tuberculosis</p>
                  <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-accent/15 text-accent border border-accent/30">Strong (WHO 2022 / ATS-CDC-IDSA)</span>
                </div>
                <p className="text-muted-foreground"><strong>Start when:</strong> AFB smear positive, <strong>GeneXpert MTB/RIF (Ultra) positive</strong>, urine LAM positive (HIV CD4 &lt;100), or strong clinical/radiological suspicion in an immunosuppressed patient — do not delay for culture.<sup><a href="#ref-30" className="text-primary hover:underline">30</a>,<a href="#ref-31" className="text-primary hover:underline">31</a>,<a href="#ref-32" className="text-primary hover:underline">32</a></sup> Negative-pressure isolation; notify public health. <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-accent/15 text-accent border border-accent/30">Strong</span></p>
                <p className="text-muted-foreground"><strong>1st-line (drug-sensitive):</strong> <strong>RIPE</strong> ×2 mo intensive: <strong>Rifampicin 10 mg/kg (max 600 mg) OD, Isoniazid 5 mg/kg (max 300 mg) OD + pyridoxine 10–25 mg, Pyrazinamide 25 mg/kg OD, Ethambutol 15 mg/kg OD</strong>; then <strong>RI ×4 mo</strong>.<sup><a href="#ref-30" className="text-primary hover:underline">30</a>,<a href="#ref-31" className="text-primary hover:underline">31</a></sup> TB meningitis / disseminated → 9–12 mo + adjuvant <strong>dexamethasone 0.3–0.4 mg/kg/day taper over 6–8 wk</strong> (Thwaites). <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-accent/15 text-accent border border-accent/30">Strong</span></p>
                <p className="text-muted-foreground"><strong>Monitor:</strong> baseline + 2-weekly <strong>LFTs</strong> (DILI ~10%; stop RIPE if ALT &gt;5× ULN or symptomatic &gt;3×); visual acuity + colour (ethambutol), uric acid (pyrazinamide); HIV test + DST in all;<sup><a href="#ref-30" className="text-primary hover:underline">30</a>,<a href="#ref-31" className="text-primary hover:underline">31</a></sup> rifampicin CYP interactions; sputum smear/culture at 2, 5, 6 mo; watch for IRIS at 2–8 wk on ART (consider prednisolone 1 mg/kg taper). <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-primary/10 text-primary border border-primary/30">Conditional (IRIS)</span></p>
                <p className="text-muted-foreground"><strong>Stop / step-down:</strong> never stop on early cultures (TB takes 2–8 wk); complete full course.<sup><a href="#ref-30" className="text-primary hover:underline">30</a></sup> Switch to <strong>BPaL/BPaLM</strong> (bedaquiline + pretomanid + linezolid ± moxifloxacin) if Xpert shows rifampicin resistance — refer TB MDT.<sup><a href="#ref-30" className="text-primary hover:underline">30</a></sup> Stop empirical Rx only if alternative diagnosis confirmed <em>and</em> ≥3 cultures + Xpert negative <em>and</em> radiology improving off treatment. <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-accent/15 text-accent border border-accent/30">Strong</span></p>
              </div>

              {/* Bacterial superinfection */}
              <div className="rounded-lg border border-border bg-card p-3 text-xs space-y-2">
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  <p className="font-semibold text-foreground text-sm">Bacterial superinfection (HAP/VAP overlay)</p>
                  <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-accent/15 text-accent border border-accent/30">Strong (IDSA-ATS 2016 / SSC 2021)</span>
                </div>
                <p className="text-muted-foreground"><strong>Start when:</strong> new fever spike, purulent secretions, new lobar consolidation, septic shock, rising <strong>procalcitonin &gt;0.5 µg/L</strong> with rising CRP/WCC, or BAL ≥10⁵ CFU/mL (≥10⁴ if PSB) on quantitative culture.<sup><a href="#ref-33" className="text-primary hover:underline">33</a>,<a href="#ref-34" className="text-primary hover:underline">34</a></sup> Suspect early in mechanically ventilated PCP-ARDS &gt;48 h. <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-primary/10 text-primary border border-primary/30">Conditional (PCT)</span></p>
                <p className="text-muted-foreground"><strong>1st-line empirical (per local antibiogram):</strong> <strong>piperacillin–tazobactam 4.5 g IV QDS</strong> (extended infusion 4 h) <em>or</em> <strong>meropenem 1–2 g TDS</strong> if ESBL risk.<sup><a href="#ref-33" className="text-primary hover:underline">33</a>,<a href="#ref-34" className="text-primary hover:underline">34</a></sup> Add <strong>vancomycin 25–30 mg/kg load → 15–20 mg/kg BD (TDM AUC₂₄ 400–600)</strong> or <strong>linezolid 600 mg BD</strong> if MRSA risk. Atypical cover (azithromycin / levofloxacin) if Legionella positive or non-responding at 48 h. First dose within <strong>1 h</strong> of recognition.<sup><a href="#ref-34" className="text-primary hover:underline">34</a></sup> <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-accent/15 text-accent border border-accent/30">Strong</span></p>
                <p className="text-muted-foreground"><strong>Monitor:</strong> daily clinical review + SOFA trend; PCT day 0, 3, 5 (≥80% fall supports de-escalation);<sup><a href="#ref-35" className="text-primary hover:underline">35</a></sup> vancomycin TDM; renal function daily; <em>C. difficile</em> stool PCR if diarrhoea; review lines for source control. <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-primary/10 text-primary border border-primary/30">Conditional</span></p>
                <p className="text-muted-foreground"><strong>Stop / step-down:</strong> de-escalate at <strong>48–72 h</strong> on cultures + sensitivities + clinical response; stop MRSA / atypical cover if negative.<sup><a href="#ref-33" className="text-primary hover:underline">33</a></sup> Total duration <strong>7 days</strong> for HAP/VAP unless non-fermenting GNB, bacteraemia, immunosuppression or slow response (then 8–14 d).<sup><a href="#ref-33" className="text-primary hover:underline">33</a></sup> Stop entirely if <strong>PCT &lt;0.25 µg/L</strong> <em>and</em> cultures negative at 72 h <em>and</em> alternative explanation found (SAPS trial).<sup><a href="#ref-35" className="text-primary hover:underline">35</a></sup> <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-accent/15 text-accent border border-accent/30">Strong</span></p>
              </div>
            </div>
          ...
          <ExamSection id="endocarditis" exams={[Exam.FINAL, Exam.FFICM, Exam.EDIC]}>
            <CollapsibleSubsection title="Infective Endocarditis">
            <p className="text-muted-foreground leading-relaxed mb-3">
          ...
          <ExamSection id="viral" exams={[Exam.FINAL, Exam.FFICM, Exam.EDIC]}>
            <CollapsibleSubsection title="Viral Infections in ICU">
            <div className="overflow-x-auto">
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr className="border-b border-border">
          ...
          <ExamSection id="mdr" exams={[Exam.FFICM, Exam.EDIC]} curriculumCodes={["FFICM 4.7"]}>
            <CollapsibleSubsection title="Drug-Resistant Organisms">
            <p className="text-muted-foreground leading-relaxed mb-3">
          ...
          <ExamSection id="leptospirosis" exams={[Exam.FINAL, Exam.FFICM, Exam.EDIC]}>
            <CollapsibleSubsection title="Leptospirosis">
            <p className="text-muted-foreground leading-relaxed mb-3">
          ...
          <ExamSection id="control" exams={[Exam.FINAL, Exam.FFICM, Exam.EDIC]}>
            <CollapsibleSubsection title="Infection Control & Prevention">
            <div className="space-y-2">
              {[
