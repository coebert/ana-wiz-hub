      ...
      coreConcepts={
        <>
          ...
          <ExamSection id="fungal" exams={[Exam.FINAL, Exam.FFICM, Exam.EDIC]}>
            ...
            <p className="text-xs text-muted-foreground mb-2">Empirical pathways to run <em>in parallel</em> with co-trimoxazole when the corresponding co-pathogen is suspected. Each pathway lists the trigger to start, first-line dose, what to monitor, and the rule for stopping once microbiology returns.</p>
            <div className="grid md:grid-cols-2 gap-3 mb-4">
              ...
              <div className="rounded-lg border border-border bg-card p-3 text-xs space-y-2">
                ...
                <p className="text-muted-foreground"><strong>Start when:</strong> BAL CMV PCR positive <em>and</em> compatible CT (diffuse GGO, nodules) <em>and</em> immunosuppressed host (HSCT, SOT, advanced HIV with CD4 &lt;100, biologics) — or rising blood CMV viral load &gt;1000 IU/mL with hypoxia despite anti-PCP therapy. BAL PCR alone may reflect shedding — clinical-radiological correlation essential.</p>
                <p className="text-muted-foreground"><strong>1st-line:</strong> IV <strong>ganciclovir 5 mg/kg BD</strong> (induction 14–21 d, then maintenance 5 mg/kg OD); switch to oral <strong>valganciclovir 900 mg BD</strong> when stable and absorbing. <strong>Foscarnet 60 mg/kg TDS</strong> if myelosuppression, ganciclovir resistance (UL97 mutation), or concurrent etoposide. Consider CMV hyperimmune globulin in HSCT pneumonitis.</p>
                <p className="text-muted-foreground"><strong>Monitor:</strong> blood CMV PCR twice weekly (target ≥1 log drop by day 7, undetectable by day 14–21); FBC for neutropenia (ganciclovir) — pause/reduce if ANC &lt;0.5; U&amp;E and ionised Ca²⁺/Mg²⁺ daily on foscarnet (nephrotoxicity, electrolyte chelation); LFTs weekly.</p>
                <p className="text-muted-foreground"><strong>Stop / step-down:</strong> after ≥14 d induction <em>and</em> two consecutive undetectable blood PCRs (≥1 wk apart) <em>and</em> clinical/radiological resolution. Continue secondary prophylaxis (valganciclovir 900 mg OD) until immune reconstitution (CD4 &gt;100 × 3 mo in HIV; per local SOT/HSCT protocol). Stop early if PCR negative on repeat BAL <em>and</em> alternative cause confirmed.</p>
              </div>

              {/* Invasive aspergillosis */}
              <div className="rounded-lg border border-border bg-card p-3 text-xs space-y-2">
                <p className="font-semibold text-foreground text-sm">Invasive pulmonary aspergillosis (IPA)</p>
                <p className="text-muted-foreground"><strong>Start when:</strong> probable/proven IPA by EORTC/MSGERC 2020 (host factor + clinical feature + mycological evidence): serum galactomannan ≥0.5 OD <em>or</em> BAL galactomannan ≥1.0 OD, BAL <em>Aspergillus</em> PCR positive, or positive culture/histology. Suspect in haematological malignancy/neutropenia &gt;10 d, allogeneic HSCT, prolonged high-dose steroids, advanced HIV, severe COVID/influenza (CAPA / IAPA), structural lung disease, or HRCT halo / air-crescent / cavity.</p>
                <p className="text-muted-foreground"><strong>1st-line:</strong> IV <strong>voriconazole 6 mg/kg BD ×2 doses load → 4 mg/kg BD</strong> (or <strong>isavuconazole 200 mg TDS ×6 doses → 200 mg OD</strong> — preferred if QTc long, hepatic dysfunction or renal failure). Reserve <strong>liposomal amphotericin B 3 mg/kg/day</strong> for azole intolerance or proven azole-resistant <em>A. fumigatus</em>. Combination therapy (voriconazole + echinocandin) only as salvage.</p>
                <p className="text-muted-foreground"><strong>Monitor:</strong> voriconazole TDS — trough <strong>1–5.5 mg/L</strong> at day 5 and after dose change (toxicity at &gt;5.5: visual hallucinations, hepatotoxicity, periostitis); QTc, LFTs twice-weekly; vision; CYP interactions (avoid rifampicin, phenytoin). Twice-weekly serum galactomannan as response marker (≥1 log fall by day 14 = good response). Repeat HRCT at 7–14 d.</p>
                <p className="text-muted-foreground"><strong>Stop / step-down:</strong> minimum <strong>6–12 weeks</strong>; continue until clinical + radiological resolution <em>and</em> reversal of immunosuppression / neutrophil recovery. Step down to oral voriconazole/isavuconazole when stable. Stop if mycology repeatedly negative <em>and</em> alternative diagnosis established <em>and</em> immune status restored.</p>
              </div>

              {/* TB */}
              <div className="rounded-lg border border-border bg-card p-3 text-xs space-y-2">
                <p className="font-semibold text-foreground text-sm">Pulmonary tuberculosis</p>
                <p className="text-muted-foreground"><strong>Start when:</strong> AFB smear positive, <strong>GeneXpert MTB/RIF (Ultra) positive</strong>, urine LAM positive (HIV with CD4 &lt;100), or strong clinical/radiological suspicion (upper-lobe cavitation, miliary pattern, weight loss, endemic exposure) in an immunosuppressed patient — do not delay treatment for culture. Place in negative-pressure isolation; notify public health.</p>
                <p className="text-muted-foreground"><strong>1st-line (drug-sensitive):</strong> <strong>RIPE</strong> ×2 mo intensive phase: <strong>Rifampicin 10 mg/kg (max 600 mg) OD, Isoniazid 5 mg/kg (max 300 mg) OD + pyridoxine 10–25 mg, Pyrazinamide 25 mg/kg OD, Ethambutol 15 mg/kg OD</strong>; then <strong>RI ×4 mo</strong> continuation phase. TB meningitis / disseminated → extend to 9–12 mo + adjuvant <strong>dexamethasone 0.3–0.4 mg/kg/day tapered over 6–8 wk</strong> (Thwaites). Use IV/NG if absorption unreliable in ICU.</p>
                <p className="text-muted-foreground"><strong>Monitor:</strong> baseline + 2-weekly <strong>LFTs</strong> (DILI rate ~10%; stop all RIPE if ALT &gt;5× ULN or symptomatic &gt;3×); visual acuity + colour (ethambutol), uric acid (pyrazinamide); HIV test, drug-susceptibility testing in all; rifampicin CYP interactions (review every drug — esp. azoles, anticoagulants, ART); sputum smear/culture at 2, 5 and 6 mo for response; watch for paradoxical reaction / IRIS (worse in 2–8 wk after starting ART — consider prednisolone 1 mg/kg taper).</p>
                <p className="text-muted-foreground"><strong>Stop / step-down:</strong> never stop based on early cultures — TB cultures take 2–8 wk; continue full course. Adjust to <strong>MDR-TB regimen</strong> (bedaquiline + linezolid + pretomanid ± moxifloxacin per BPaL/BPaLM, refer TB MDT) if rifampicin resistance on Xpert. Stop empirical therapy only if alternative diagnosis confirmed <em>and</em> ≥3 cultures + Xpert negative <em>and</em> radiology improving on no TB treatment.</p>
              </div>

              {/* Bacterial superinfection */}
              <div className="rounded-lg border border-border bg-card p-3 text-xs space-y-2">
                <p className="font-semibold text-foreground text-sm">Bacterial superinfection (HAP/VAP overlay)</p>
                <p className="text-muted-foreground"><strong>Start when:</strong> new fever spike, purulent secretions, new lobar consolidation, septic shock, rising <strong>procalcitonin &gt;0.5 µg/L</strong> with rising CRP/WCC, or BAL with ≥10⁵ CFU/mL (≥10⁴ if PSB) on quantitative culture. Suspect early in mechanically ventilated PCP-ARDS &gt;48 h.</p>
                <p className="text-muted-foreground"><strong>1st-line empirical (per Surviving Sepsis 2021 + local antibiogram):</strong> <strong>piperacillin–tazobactam 4.5 g IV QDS</strong> (extended infusion 4 h) <em>or</em> <strong>meropenem 1–2 g TDS</strong> if ESBL risk. Add <strong>vancomycin 25–30 mg/kg load → 15–20 mg/kg BD (TDM trough 15–20 mg/L)</strong> or <strong>linezolid 600 mg BD</strong> if MRSA risk (recent admission, colonised, IVDU, severe sepsis). Add atypical cover (azithromycin / levofloxacin) for severe CAP overlap, Legionella antigen positive, or no improvement at 48 h. First dose within <strong>1 h</strong> of recognition.</p>
                <p className="text-muted-foreground"><strong>Monitor:</strong> daily clinical review + SOFA trend; PCT day 0, 3, 5 (≥80% fall supports de-escalation); repeat cultures only if not improving; vancomycin TDM (trough or AUC₂₄ 400–600); renal function daily (β-lactam nephrotoxicity, vancomycin AKI); <em>Clostridioides difficile</em> stool PCR if diarrhoea; review lines for source control.</p>
                <p className="text-muted-foreground"><strong>Stop / step-down:</strong> de-escalate at <strong>48–72 h</strong> based on cultures, sensitivities, and clinical response — narrow to the susceptible agent, stop MRSA/atypical cover if negative. Total duration <strong>7 days</strong> for HAP/VAP (PneumA / IDSA-ATS 2016) unless non-fermenting GNB (<em>Pseudomonas</em>, <em>Acinetobacter</em>), bacteraemia, immunosuppression, or slow response (then 8–14 d). Stop entirely if PCT &lt;0.25 µg/L <em>and</em> cultures negative at 72 h <em>and</em> alternative explanation found (PCT-guided cessation reduces antibiotic days, Crit Care Med 2018).</p>
              </div>
            </div>
