...
import PVLoopDiagram from "@/components/diagrams/physiology/PVLoopDiagram";
import FrankStarlingDiagram from "@/components/diagrams/physiology/FrankStarlingDiagram";
import CardiacActionPotentialDiagram from "@/components/diagrams/physiology/CardiacActionPotentialDiagram";
import AorticDicroticNotchDiagram from "@/components/diagrams/physiology/AorticDicroticNotchDiagram";
import DichroticNotchComparisonPanel from "@/components/diagrams/physiology/DichroticNotchComparisonPanel";
  ...
  return (
    <TopicTemplate
      ...
      keyPoints={[
        ...
        { text: "Isovolumetric contraction and relaxation: all valves closed; pressure changes without volume change.", cites: ["Ganong Ch.30"] },
        { text: "Passive filling contributes 70–80% of LVEDV; atrial kick adds 15–25% — critical in stiff ventricles (AS, HCM, diastolic dysfunction).", cites: ["BJA Educ 2018"] },
        { text: "SV determinants: preload, afterload, contractility; CO = HR × SV; rhythm contributes via AV synchrony.", cites: ["Power & Kam Ch.4"] },
        { text: "LA/CVP trace: a (atrial contraction), c (AV bulge), v (atrial filling), x and y descents — interpret abnormalities at the bedside.", cites: ["Ganong Ch.30"] },
        { text: "Cannon a = AV dissociation; giant a = TS/PHT; giant v = MR/TR; steep y = constriction; blunted y = tamponade.", cites: ["BJA Educ 2018"] },
        { text: "Left coronary perfusion is diastolic — CPP ≈ aortic DBP − LVEDP; protect with rate control, adequate DBP, low LVEDP.", cites: ["Power & Kam Ch.4"] },
        { text: "Right heart operates at low pressure; RV is afterload-sensitive — small ↑PVR markedly reduces RV SV (PE, hypoxia, acidosis).", cites: ["Ganong Ch.30"] },
        { text: "Valvular lesions each have characteristic loop and waveform changes that drive specific anaesthetic targets (HR, SVR, preload, rhythm).", cites: ["BJA Educ 2018"] },
      ]}
      coreConcepts={
        <>
          ...
          <ExamSection id="sv-determinants" exams={[Exam.PRIMARY, Exam.FINAL]} curriculumCodes={["CR_BK_01"]}>
            ...
            <div className="mt-4 space-y-4">
              ...
              <div className="rounded-lg border border-border p-4 text-sm text-foreground/85"><h3 className="font-semibold text-foreground">PV loops in valvular disease</h3><ul className="mt-2 space-y-1"><li><strong>Aortic stenosis:</strong> tall, narrow loop — very high peak LV pressure with a large transvalvular gradient, concentric hypertrophy steepening the end-diastolic pressure–volume relationship, and stroke volume relatively preserved until late. Stroke work rises markedly for a given output.</li><li><strong>Aortic regurgitation:</strong> wide loop shifted right (large EDV) with loss of the true isovolumetric relaxation phase, because filling begins as soon as LV pressure falls below aortic pressure.</li><li><strong>Mitral regurgitation:</strong> loop shifted right with loss of isovolumetric contraction — ejection into the left atrium starts before the aortic valve opens, so the loop has a rounded, triangular lower-right corner and reduced effective forward stroke volume.</li><li><strong>Mitral stenosis:</strong> small, narrow loop with reduced EDV and low stroke work; output is fixed and rate-dependent.</li></ul><p className="mt-2">Match each pattern to the anaesthetic priorities in the Applied Pathophysiology table below <InlineRef topicId="cardiac-cycle" refLabel="BJA Educ 2018" />.</p></div>
              <div className="rounded-lg border border-border p-4 text-sm text-foreground/85"><h3 className="font-semibold text-foreground">Dynamic indices & pulse wave analysis</h3><ul className="mt-2 space-y-1"><li><strong>PPV</strong> = (PP<sub>max</sub> − PP<sub>min</sub>)/PP<sub>mean</sub> × 100; values above roughly 12–13% suggest fluid responsiveness in a sedated, ventilated patient in sinus rhythm with V<sub>T</sub> ≥ 8 mL/kg.</li><li><strong>SVV</strong> = (SV<sub>max</sub> − SV<sub>min</sub>)/SV<sub>mean</sub> × 100, derived from calibrated or uncalibrated pulse-contour stroke volume. It shares the same physiology and threshold range as PPV but depends on the device algorithm; PPV needs only an arterial line, whereas SVV needs a stroke-volume estimate and is more affected by vasomotor tone and waveform damping.</li><li><strong>dP/dt<sub>max</sub></strong> — maximal rate of pressure rise during isovolumetric contraction, an index of contractility. Estimated from the upstroke of the arterial trace, so it is confounded by afterload, arterial compliance and catheter damping; trend it rather than trusting absolute values.</li><li><strong>Cardiac power output</strong> = MAP × CO / 451 (watts; normal ≈ 1 W). CPO below about 0.6 W is a strong predictor of mortality in cardiogenic shock and is the single most useful haemodynamic prognostic variable.</li><li><strong>Cardiac cycle efficiency</strong> — a dimensionless pulse-wave-analysis variable describing the energetic efficiency of each beat; negative values correlate with impaired LV ejection fraction.</li></ul><p className="mt-2">Pulse wave analysis derives stroke volume beat-to-beat from the arterial pressure waveform (pulse-contour, pressure-recording analytical or Windkessel models), then computes CO, SVV and derived efficiency/contractility indices. Accuracy falls with arrhythmia, aortic regurgitation, intra-aortic balloon counterpulsation, extreme vasoconstriction or poor waveform quality <InlineRef topicId="cardiac-cycle" refLabel="ICM 2013 Pulse Wave" />.</p></div>
              <div className="bg-card rounded-xl border border-border p-4 md:p-6">
