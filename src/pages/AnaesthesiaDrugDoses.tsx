...
import {
  ...
  type AnaesthesiaDrugDose,
} from "@/data/anaesthesiaDrugDoses";

const TopicLinks = ({ paths }: { paths?: string[] }) => {
    ...
    <span className="mt-1.5 flex flex-wrap gap-1.5">
      {paths.map((p) => (
        <Link
          key={p}
          to={p}
          className="rounded-full border border-perioperative/30 bg-perioperative/5 px-2 py-0.5 text-xs text-perioperative underline-offset-2 hover:underline"
        >
...
const AnaesthesiaDrugDoses = () => {
  const [search, setSearch] = useState("");
  const [activeGroup, setActiveGroup] = useState<string>("all");

  const groups = useMemo(() => {
    const q = search.trim().toLowerCase();
    return anaesthesiaDrugDoseGroups
    ...
    <main className="min-h-screen bg-background">
      ...
      <PageSection className="pt-8 pb-16">
        ...
        <div className="mt-4 flex items-start gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Anaesthesia Drug Dosing Table</h1>
            <p className="mt-2 max-w-2xl text-muted-foreground">
              Dose, route, frequency and key indications for the drugs of everyday
              anaesthetic practice — induction, maintenance, analgesia, muscle
              relaxants and their reversal — grouped by clinical purpose and
              linked to the relevant topics for revision and viva preparation.
            </p>
          </div>
        </div>
        ...
        <div className="mt-5 flex items-start gap-2.5 rounded-lg border border-border bg-muted/40 p-4 text-sm text-muted-foreground">
          <TriangleAlert className="mt-0.5 h-4 w-4 shrink-0 text-perioperative" aria-hidden />
          <p>
            Revision aid only. Doses are typical adult starting points assuming a
            70 kg patient with normal organ function; always check the BNF, the
            product literature and your local protocols before prescribing, and
            adjust for weight, age, pregnancy, and renal and hepatic function.
          </p>
        </div>
        ...
        <div className="relative mt-6 max-w-md">
          ...
          <Input
            ...
            placeholder="Search drug, indication or dose (e.g. sugammadex, PONV, RSI)…"
            className="pl-9"
            aria-label="Search anaesthesia drug doses"
          />
        </div>
        ...
        <div className="mt-4 flex gap-2 overflow-x-auto pb-1">
          ...
          {anaesthesiaDrugDoseGroups.map((g) => (
            <Button
              key={g.id}
              size="sm"
        ...
        <p className="mt-3 text-sm text-muted-foreground">
          {shown} of {anaesthesiaDrugCount} drugs shown
        </p>
            ...
            <section key={group.id} id={group.id} className="scroll-mt-24">
              ...
              <div className="mt-4 hidden overflow-x-auto rounded-xl border border-border md:block">
                <table className="w-full min-w-[720px] border-collapse text-sm">
                  ...
                  <tbody>
                    {group.drugs.map((d: AnaesthesiaDrugDose) => (
        ...
        <p className="mt-10 text-sm text-muted-foreground">
          Working in critical care? See the{" "}
          <Link
            to="/intensive-care/drug-doses"
            className="font-medium text-perioperative underline-offset-4 hover:underline"
          >
          ...
          for vasopressor, inotrope and sedation infusions.
        </p>
      </PageSection>
    </main>
  );
...
export default AnaesthesiaDrugDoses;
