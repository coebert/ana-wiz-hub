export interface DrugDose {
  drug: string;
  dose: string;
  route: string;
  frequency: string;
  indications: string;
  notes?: string;
  /** Typical dose for infants and children (per kg, adult ceiling applies). */
  paediatricDose?: string;
  /** Dose for term neonates and infants under about 1 month, where different. */
  neonatalDose?: string;
  /** Paediatric-specific cautions. */
  paediatricNotes?: string;
  topicIds?: string[];
}

export interface DrugDoseGroup {
  id: string;
  title: string;
  blurb: string;
  drugs: DrugDose[];
}

/**
 * Adult ICU dosing reference. Doses are typical adult starting points for a
 * 70 kg patient with normal organ function and must always be checked against
 * the BNF, local critical care guidelines and the individual patient.
 */
export const icuDrugDoseGroups: DrugDoseGroup[] = [
  {
    id: "sedation-analgesia",
    title: "Sedation, analgesia and delirium",
    blurb: "Titrate to a defined sedation score (e.g. RASS 0 to −2) with daily interruption or lightening; analgesia first, then sedation.",
    drugs: [
      {
        drug: "Propofol 1–2%",
        dose: "0.3–4 mg/kg/h (bolus 0.5–1 mg/kg for procedures)",
        route: "IV infusion (central preferred)",
        frequency: "Continuous",
        indications: "Short-term sedation of the ventilated patient; status epilepticus; rapid neurological wake-up testing",
        notes: "Keep below 4 mg/kg/h and review after 48 h — propofol-related infusion syndrome (lactic acidosis, rhabdomyolysis, arrhythmia). Provides 1.1 kcal/mL as lipid; causes vasodilatation and myocardial depression.",
        topicIds: ["icu-sedation-delirium", "intravenous-anaesthetics"],
        paediatricDose: "Sedation 1–4 mg/kg/h; procedural bolus 1–2 mg/kg",
        neonatalDose: "Avoid — not licensed for neonatal/paediatric ICU sedation",
        paediatricNotes: "Prolonged high-dose infusion in children carries a high PRIS risk; use for short-term sedation only and monitor lactate and CK.",
      },
      {
        drug: "Fentanyl",
        dose: "25–200 microgram/h (bolus 25–100 microgram)",
        route: "IV infusion",
        frequency: "Continuous",
        indications: "Analgesia and ventilator tolerance",
        notes: "Highly lipid soluble — accumulates in prolonged infusions with a context-sensitive half-time that lengthens markedly.",
        topicIds: ["icu-sedation-delirium", "opioids"],
        paediatricDose: "Bolus 1–2 microgram/kg; infusion 1–4 microgram/kg/h",
        neonatalDose: "Bolus 0.5–1 microgram/kg; infusion 0.5–2 microgram/kg/h",
        paediatricNotes: "Give neonatal boluses slowly over 5 min — rapid injection causes chest-wall rigidity and bradycardia.",
      },
      {
        drug: "Morphine",
        dose: "1–5 mg IV bolus; 1–5 mg/h infusion",
        route: "IV",
        frequency: "Hourly PRN or continuous",
        indications: "Analgesia, dyspnoea at end of life",
        notes: "Active metabolite morphine-6-glucuronide accumulates in renal failure — reduce dose or switch to fentanyl/alfentanil.",
        topicIds: ["opioids", "end-of-life-care"],
        paediatricDose: "Bolus 50–100 microgram/kg; infusion 10–40 microgram/kg/h",
        neonatalDose: "Bolus 25–50 microgram/kg; infusion 5–10 microgram/kg/h",
        paediatricNotes: "Neonatal clearance is immature (glucuronidation matures over the first months) — halve the dose and monitor apnoea.",
      },
      {
        drug: "Alfentanil",
        dose: "0.5–2 mg/h",
        route: "IV infusion",
        frequency: "Continuous",
        indications: "Analgesia in renal failure; short procedures",
        notes: "No active metabolites; offset largely independent of renal function.",
        topicIds: ["opioids", "aki-rrt"],
        paediatricDose: "Infusion 10–60 microgram/kg/h ventilated",
        neonatalDose: "10–20 microgram/kg/h; specialist use only",
        paediatricNotes: "Short context-sensitive half-time makes it useful for neonatal ventilation and procedures.",
      },
      {
        drug: "Midazolam",
        dose: "1–10 mg/h (bolus 1–5 mg)",
        route: "IV infusion",
        frequency: "Continuous",
        indications: "Sedation where haemodynamic instability limits propofol; seizure control; alcohol withdrawal",
        notes: "Benzodiazepine sedation is associated with more delirium and longer ventilation; active metabolite accumulates in renal failure.",
        topicIds: ["icu-sedation-delirium", "benzodiazepines"],
        paediatricDose: "Bolus 100 microgram/kg; infusion 60–240 microgram/kg/h (1–4 microgram/kg/min)",
        neonatalDose: "Infusion 30–60 microgram/kg/h; avoid boluses",
        paediatricNotes: "Boluses cause hypotension in neonates; infusion is linked to withdrawal and delirium — wean over days after prolonged use.",
      },
      {
        drug: "Dexmedetomidine",
        dose: "0.2–1.4 microgram/kg/h (no loading bolus in ICU)",
        route: "IV infusion",
        frequency: "Continuous",
        indications: "Light, cooperative sedation; weaning agitation; hyperactive delirium",
        notes: "Central α2 agonist — causes bradycardia and hypotension; does not depress respiration.",
        topicIds: ["icu-sedation-delirium"],
        paediatricDose: "0.2–1.4 microgram/kg/h; no loading dose",
        neonatalDose: "0.05–0.3 microgram/kg/h; limited data",
        paediatricNotes: "Useful for extubation and delirium sparing; watch bradycardia, especially in infants.",
      },
      {
        drug: "Clonidine",
        dose: "0.5–2 microgram/kg/h IV; 50–100 microgram TDS enterally",
        route: "IV infusion or enteral",
        frequency: "Continuous or 8-hourly",
        indications: "Sedation sparing, opioid and alcohol withdrawal",
        notes: "Rebound hypertension on abrupt withdrawal.",
        topicIds: ["icu-sedation-delirium"],
        paediatricDose: "0.1–2 microgram/kg/h IV, or 1–5 microgram/kg 6-hourly enterally",
        neonatalDose: "0.1–0.5 microgram/kg/h",
        paediatricNotes: "Widely used in PICU as an opioid/benzodiazepine-sparing agent and for withdrawal; rebound hypertension on abrupt stop.",
      },
      {
        drug: "Ketamine",
        dose: "0.1–0.5 mg/kg/h (bolus 0.25–1 mg/kg)",
        route: "IV",
        frequency: "Continuous or PRN",
        indications: "Analgesia-sparing sedation, severe bronchospasm, opioid-tolerant pain, cardiostable induction",
        notes: "Preserves airway reflexes and sympathetic tone; may cause emergence phenomena and hypersalivation.",
        topicIds: ["intravenous-anaesthetics", "bronchospastic-failure"],
        paediatricDose: "Induction 1–2 mg/kg IV; infusion 0.5–2 mg/kg/h",
        neonatalDose: "Induction 0.5–1 mg/kg; infusion 0.3–0.6 mg/kg/h",
        paediatricNotes: "Preserves airway tone and drive; first choice for the shocked or asthmatic child.",
      },
      {
        drug: "Haloperidol",
        dose: "0.5–2.5 mg, up to 10 mg/24 h in most ICU protocols",
        route: "IV or oral",
        frequency: "PRN 4–6 hourly",
        indications: "Distressing hyperactive delirium after non-pharmacological measures",
        notes: "Monitor QTc; avoid in Parkinson's disease and prolonged QT. Does not prevent or shorten delirium.",
        topicIds: ["icu-sedation-delirium"],
        paediatricDose: "10–25 microgram/kg 8–12 hourly (max 5 mg/dose)",
        neonatalDose: "Avoid",
        paediatricNotes: "Rarely used; ECG monitoring for QT prolongation and dystonic reactions in children.",
      },
      {
        drug: "Remifentanil",
        dose: "0.05–0.25 microgram/kg/min (about 3–15 microgram/kg/h); analgesia-based sedation usually 0.05–0.15 microgram/kg/min",
        route: "IV infusion (dedicated lumen, never bolused on the ICU)",
        frequency: "Continuous",
        indications: "Analgesia-based sedation where rapid, predictable waking is needed — neuro observation, difficult weaning, hepatic or renal failure",
        notes: "Ester hydrolysis by non-specific plasma and tissue esterases: context-sensitive half-time of 3–4 min however long the infusion runs, and unchanged in renal or hepatic failure. Offset is immediate, so a longer-acting analgesic must be given before it is stopped or the patient will be in pain within minutes. Causes bradycardia, hypotension, chest-wall rigidity with boluses and acute opioid tolerance/hyperalgesia after prolonged high doses.",
        topicIds: ["opioids", "icu-sedation-delirium", "tiva"],
        paediatricDose: "0.05–0.3 microgram/kg/min",
        neonatalDose: "0.03–0.1 microgram/kg/min (specialist use; apnoea and rigidity with rapid increases)",
        paediatricNotes: "Clearance per kilogram is higher in infants, so weight-based rates are higher than in adults; always plan the follow-on analgesia before stopping.",
      },
      {
        drug: "Oxycodone",
        dose: "Immediate-release 5–10 mg orally 4–6 hourly PRN; IV 1–2 mg increments to 10 mg (0.05–0.1 mg/kg)",
        route: "Oral, enteral tube or IV",
        frequency: "4–6 hourly PRN or regular with PRN cover",
        indications: "Step-down analgesia when weaning an IV opioid infusion; enteral analgesia in the recovering ICU patient",
        notes: "Roughly twice as potent as oral morphine and less dependent on active metabolites, so it is often preferred in renal impairment — but oxymorphone and noroxycodone still accumulate, so halve the dose when eGFR is under 30 or in hepatic impairment. Metabolised by CYP3A4 and CYP2D6: levels rise with clarithromycin, fluconazole and ritonavir.",
        topicIds: ["opioids", "pain-medicine"],
        paediatricDose: "100–200 microgram/kg orally 4–6 hourly (max 5 mg per dose in younger children)",
        neonatalDose: "Not recommended",
        paediatricNotes: "Reserve for children over 1 month under specialist advice; monitor sedation score and respiratory rate after each dose change.",
      },
      {
        drug: "Methadone",
        dose: "Enteral 2.5–10 mg 8–12 hourly, titrated slowly; opioid substitution doses continued after confirmation with the community pharmacy",
        route: "Oral or enteral tube (IV under specialist advice)",
        frequency: "8–12 hourly, or once daily for substitution therapy",
        indications: "Weaning prolonged opioid infusions and preventing iatrogenic withdrawal; continuation of opioid substitution therapy; opioid-tolerant patients",
        notes: "Long and highly variable half-life (15–60 h) means the full effect of a dose change appears only after 3–5 days — increase no more often than every 3 days. NMDA antagonism helps opioid-tolerant and neuropathic pain. Prolongs QT: check an ECG at baseline and after dose increases, and avoid other QT-prolonging drugs.",
        topicIds: ["opioids", "icu-sedation-delirium", "pain-medicine"],
        paediatricDose: "100–200 microgram/kg 8–12 hourly for opioid weaning, reduced by 10–20% per day",
        neonatalDose: "50–100 microgram/kg 12-hourly (neonatal abstinence and post-ECMO weaning, specialist only)",
        paediatricNotes: "The standard PICU choice for weaning long fentanyl or morphine infusions; use a withdrawal score (WAT-1 or SOS) to guide the taper.",
      },
      {
        drug: "Paracetamol",
        dose: "1 g 6-hourly (maximum 4 g/24 h); 15 mg/kg 6-hourly and maximum 60 mg/kg/24 h if under 50 kg, malnourished, in hepatic impairment or chronic alcohol excess",
        route: "IV, oral or enteral tube",
        frequency: "6-hourly",
        indications: "Baseline multimodal analgesia and opioid sparing; antipyresis where fever control is clinically indicated",
        notes: "The foundation of ICU multimodal analgesia — prescribe it regularly rather than PRN. IV doses cause transient hypotension in shocked patients. Reduce the dose in low body weight, malnutrition, sepsis-related glutathione depletion and liver disease; watch for hidden paracetamol in combination enteral preparations.",
        topicIds: ["analgesics", "pain-medicine", "acute-liver-failure"],
        paediatricDose: "15 mg/kg 6-hourly IV or oral (maximum 60 mg/kg/24 h)",
        neonatalDose: "Term neonate 10–15 mg/kg 8-hourly IV (maximum 30 mg/kg/24 h); preterm 7.5 mg/kg 8-hourly",
        paediatricNotes: "Prescribe by weight and check the 24-hour total — 10-fold and mg/mL dilution errors with IV paracetamol are a recognised paediatric never-event.",
      },
      {
        drug: "Thiopental",
        dose: "Bolus 3–5 mg/kg; infusion 1–5 mg/kg/h titrated to burst suppression",
        route: "IV infusion (central line)",
        frequency: "Continuous for barbiturate coma",
        indications: "Refractory status epilepticus and refractory intracranial hypertension after tier-two measures",
        notes: "Zero-order kinetics at high dose with saturable, prolonged elimination — after days of infusion patients may not wake for 48–96 h, which makes neurological examination and brainstem testing unreliable. Causes profound vasodilatation and myocardial depression (expect vasopressor escalation), immunosuppression, ileus and hypokalaemia, with rebound hyperkalaemia on stopping.",
        topicIds: ["status-epilepticus", "neuro-critical-care", "intravenous-anaesthetics"],
        paediatricDose: "Bolus 3–5 mg/kg; infusion 1–5 mg/kg/h to EEG burst suppression",
        neonatalDose: "Bolus 2–3 mg/kg (specialist use only)",
        paediatricNotes: "Requires continuous EEG, invasive monitoring and an inotrope-ready plan; accumulation is even more marked in infants.",
      },
      {
        drug: "Inhaled isoflurane (Sedaconda ACD-S)",
        dose: "0.3–0.7% end-tidal, titrated to sedation score (typically 3–8 mL/h liquid via the anaesthetic-conserving device)",
        route: "Inhaled via anaesthetic-conserving device in the ventilator circuit",
        frequency: "Continuous",
        indications: "Sedation of the ventilated adult where IV sedation is failing or accumulating; severe refractory bronchospasm (bronchodilator effect); status epilepticus in specialist units",
        notes: "Elimination is almost entirely pulmonary, so waking is rapid and independent of hepatic and renal function — useful when propofol has hit its ceiling or accumulated. Needs end-tidal agent monitoring, scavenging or a charcoal filter, malignant-hyperthermia screening and trained staff; watch for fluoride nephrotoxicity with very prolonged use and cost/theatre-supply issues.",
        topicIds: ["icu-sedation-delirium", "inhalational-agents", "bronchospastic-failure"],
        paediatricDose: "Specialist paediatric ICU use only, titrated to end-tidal concentration",
        neonatalDose: "Not established",
        paediatricNotes: "Device dead space is significant relative to tidal volume in small children — only use with paediatric-validated equipment.",
      },
      {
        drug: "Quetiapine",
        dose: "12.5–25 mg at night, titrated to 50–100 mg twice daily if needed",
        route: "Oral or enteral tube",
        frequency: "Nightly or 12-hourly",
        indications: "Distressing hyperactive delirium and disrupted sleep–wake cycle where non-pharmacological measures have failed; the enteral step-down from IV haloperidol",
        notes: "Sedating and less likely than haloperidol to cause acute dystonia, so it suits patients with Parkinson's disease or a prolonged QT concern — but it still prolongs QTc and causes hypotension and hyperglycaemia. It does not prevent or shorten delirium: treat pain, sepsis, hypoxia, constipation, retention and drug withdrawal first, and set a stop date so it is not carried home.",
        topicIds: ["icu-sedation-delirium"],
        paediatricDose: "Specialist psychiatric advice only",
        neonatalDose: "Not used",
        paediatricNotes: "Not a routine paediatric ICU drug — involve child psychiatry before starting an antipsychotic.",
      },
      {
        drug: "Melatonin",
        dose: "2–10 mg at 21:00 (immediate or prolonged release)",
        route: "Oral or enteral tube",
        frequency: "Once nightly",
        indications: "Sleep–wake restoration as part of a delirium-prevention bundle",
        notes: "Very well tolerated and used within the ABCDEF bundle alongside daylight, noise reduction and clustered care; evidence for preventing delirium is weak, so it supplements rather than replaces non-pharmacological measures. Stop it before discharge.",
        topicIds: ["icu-sedation-delirium"],
        paediatricDose: "2–6 mg at night (up to 10 mg in neurodisability with specialist advice)",
        neonatalDose: "Not established",
        paediatricNotes: "Commonly used in children with neurodisability or autism who already take it at home — continue their usual dose and timing.",
      },
    ],
  },
  {
    id: "neuromuscular-blockade",
    title: "Neuromuscular blockade",
    blurb: "Only with adequate sedation, a clear indication and a defined stop point; monitor with a nerve stimulator and protect the eyes.",
    drugs: [
      {
        drug: "Rocuronium",
        dose: "0.6 mg/kg intubation; 1.0–1.2 mg/kg rapid sequence; 0.3–0.6 mg/kg/h infusion",
        route: "IV",
        frequency: "Bolus or continuous",
        indications: "Intubation, severe ventilator dyssynchrony, early severe ARDS, raised ICP",
        notes: "Reversed by sugammadex 2 mg/kg (routine) or 16 mg/kg (immediate after 1.2 mg/kg).",
        topicIds: ["neuromuscular-blockers", "ards"],
        paediatricDose: "Intubation 0.6–1 mg/kg; infusion 300–600 microgram/kg/h",
        neonatalDose: "Intubation 0.45–0.6 mg/kg",
        paediatricNotes: "Onset is faster and duration longer in infants because of a larger volume of distribution; sugammadex 16 mg/kg reverses a full block.",
      },
      {
        drug: "Atracurium / cisatracurium",
        dose: "Atracurium 0.5 mg/kg then 0.3–0.6 mg/kg/h; cisatracurium 0.15 mg/kg then 1–3 microgram/kg/min",
        route: "IV",
        frequency: "Continuous",
        indications: "Prolonged blockade in hepatic or renal failure; severe ARDS",
        notes: "Hofmann elimination — organ-independent offset. Atracurium releases histamine; laudanosine accumulation is rarely clinically relevant.",
        topicIds: ["neuromuscular-blockers", "ards"],
        paediatricDose: "Atracurium 0.5 mg/kg then 300–600 microgram/kg/h; cisatracurium 0.1 mg/kg then 60–180 microgram/kg/h",
        neonatalDose: "Atracurium 0.3–0.5 mg/kg then 300 microgram/kg/h",
        paediatricNotes: "Organ-independent Hofmann elimination makes these the agents of choice in neonatal renal or hepatic failure.",
      },
      {
        drug: "Suxamethonium",
        dose: "1–1.5 mg/kg",
        route: "IV",
        frequency: "Single dose",
        indications: "Rapid sequence induction",
        notes: "Avoid after ~5 days of immobility, burns, spinal cord injury or in hyperkalaemia — risk of lethal potassium rise.",
        topicIds: ["neuromuscular-blockers"],
        paediatricDose: "1–2 mg/kg (infants and neonates 2–3 mg/kg)",
        neonatalDose: "2–3 mg/kg",
        paediatricNotes: "Give atropine 20 microgram/kg with it in infants — bradycardia is common; avoid where undiagnosed myopathy is possible.",
      },
      {
        drug: "Vecuronium",
        dose: "0.1 mg/kg intubation; 0.8–1.4 microgram/kg/min (about 0.05–0.08 mg/kg/h) infusion",
        route: "IV",
        frequency: "Bolus or continuous",
        indications: "Continued blockade where a haemodynamically neutral relaxant is wanted; alternative to rocuronium",
        notes: "Aminosteroid with an active 3-desacetyl metabolite that accumulates in renal failure — a classic cause of prolonged weakness after days of infusion. Reversible with sugammadex 2–4 mg/kg.",
        topicIds: ["neuromuscular-blockers", "aki-rrt"],
        paediatricDose: "Intubation 80–100 microgram/kg; infusion 50–100 microgram/kg/h",
        neonatalDose: "Intubation 50–100 microgram/kg",
        paediatricNotes: "Neonates are more sensitive and duration is longer; use train-of-four monitoring rather than fixed re-dosing intervals.",
      },
      {
        drug: "Pancuronium",
        dose: "0.05–0.1 mg/kg, repeated 0.01–0.02 mg/kg as needed",
        route: "IV",
        frequency: "Intermittent bolus",
        indications: "Rarely used long-acting blockade; historically for neonatal ventilation and therapeutic hypothermia",
        notes: "Vagolytic and sympathomimetic — causes tachycardia and hypertension. Renally excreted with a 2-hour half-life, so accumulation and prolonged weakness are common in critical illness; largely superseded by cisatracurium.",
        topicIds: ["neuromuscular-blockers"],
        paediatricDose: "Intubation 100 microgram/kg then 10–20 microgram/kg as required",
        neonatalDose: "Intubation 100 microgram/kg (specialist neonatal use)",
        paediatricNotes: "Duration is unpredictable in neonates; only continue with nerve-stimulator monitoring and confirmed sedation.",
      },
      {
        drug: "Mivacurium",
        dose: "0.15–0.2 mg/kg then 8–10 microgram/kg/min infusion",
        route: "IV",
        frequency: "Bolus or continuous",
        indications: "Short procedures where rapid spontaneous recovery is wanted (bronchoscopy, brief surgery on the ICU)",
        notes: "Hydrolysed by plasma cholinesterase, so recovery is 15–20 min — but block is markedly prolonged in cholinesterase deficiency and in liver failure. Histamine release causes flushing and hypotension if given fast.",
        topicIds: ["neuromuscular-blockers"],
        paediatricDose: "Bolus 100–200 microgram/kg; infusion 480–600 microgram/kg/h",
        neonatalDose: "Not routinely used",
        paediatricNotes: "Children clear it faster than adults and need proportionally higher infusion rates.",
      },
      {
        drug: "Sugammadex",
        dose: "2 mg/kg at train-of-four count ≥2; 4 mg/kg at post-tetanic count 1–2; 16 mg/kg immediately after rocuronium 1.2 mg/kg",
        route: "IV bolus",
        frequency: "Single dose (repeat 4 mg/kg if block recurs)",
        indications: "Reversal of rocuronium or vecuronium block, including can't-intubate-can't-oxygenate rescue and residual block before extubation",
        notes: "Does not reverse benzylisoquinolinium agents (atracurium, cisatracurium, mivacurium). Dose on actual body weight. Binds progesterone — treat as a missed contraceptive pill dose. Recurrence of block is possible in renal failure.",
        topicIds: ["neuromuscular-blockers", "airway-management"],
        paediatricDose: "2 mg/kg for routine reversal in children 2 years and over",
        neonatalDose: "Not licensed under 2 years — specialist decision only",
        paediatricNotes: "Reversing paralysis does not restore respiratory drive if the child is deeply sedated or obstructed — reassess the airway.",
      },
      {
        drug: "Neostigmine",
        dose: "50 microgram/kg (usually 2.5 mg) with glycopyrronium 200 microgram per 1 mg neostigmine; 2 mg by slow IV infusion over 60 min in acute colonic pseudo-obstruction",
        route: "IV",
        frequency: "Single dose, repeatable",
        indications: "Reversal of non-depolarising block at train-of-four count ≥2; acute colonic pseudo-obstruction (Ogilvie syndrome)",
        notes: "Anticholinesterase — always pair with an antimuscarinic or expect bradycardia, salivation and bronchospasm. It cannot reverse a deep block: wait for twitches. Continuous cardiac monitoring and atropine at the bedside are mandatory for the pseudo-obstruction indication.",
        topicIds: ["neuromuscular-blockers", "gastrointestinal-physiology"],
        paediatricDose: "Reversal 50 microgram/kg with glycopyrronium 10 microgram/kg",
        neonatalDose: "Reversal 50 microgram/kg with glycopyrronium 10 microgram/kg",
        paediatricNotes: "Bradycardia is pronounced in infants — give the antimuscarinic first or simultaneously.",
      },
    ],
  },
  {
    id: "vasoactive",
    title: "Vasopressors and inotropes",
    blurb: "Central administration and invasive arterial monitoring wherever possible; resuscitate volume state alongside vasoactive support.",
    drugs: [
      {
        drug: "Noradrenaline",
        dose: "0.05–1 microgram/kg/min (titrated; no ceiling in refractory shock)",
        route: "IV infusion, central (dilute peripheral acceptable short-term)",
        frequency: "Continuous",
        indications: "First-line vasopressor in septic and most vasodilatory shock; target MAP ≥65 mmHg",
        notes: "α1 >> β1. Extravasation causes local ischaemia — treat with phentolamine infiltration.",
        topicIds: ["vasoactive-agents", "sepsis", "shock"],
        paediatricDose: "0.02–1 microgram/kg/min, start 0.05 microgram/kg/min",
        neonatalDose: "0.02–0.5 microgram/kg/min",
        paediatricNotes: "Central access preferred but may be given dilute peripherally while access is obtained in shock.",
      },
      {
        drug: "Adrenaline",
        dose: "0.05–0.5 microgram/kg/min; arrest 1 mg every 3–5 min; anaphylaxis 500 microgram IM",
        route: "IV infusion, IV bolus (arrest), IM (anaphylaxis)",
        frequency: "Continuous or per protocol",
        indications: "Cardiac arrest, anaphylaxis, low cardiac output with hypotension, post-cardiotomy shock",
        notes: "Causes hyperglycaemia, hypokalaemia and a rise in lactate that does not necessarily reflect hypoperfusion.",
        topicIds: ["vasoactive-agents", "anaphylaxis", "shock"],
        paediatricDose: "Arrest 10 microgram/kg IV/IO (max 1 mg); infusion 0.02–1 microgram/kg/min",
        neonatalDose: "Resuscitation 10–30 microgram/kg IV/UVC; infusion 0.05–0.3 microgram/kg/min",
        paediatricNotes: "First-line inotrope in paediatric septic and cardiogenic shock; 10 microgram/kg is 0.1 mL/kg of 1:10,000.",
      },
      {
        drug: "Vasopressin",
        dose: "0.01–0.03 units/min (fixed, not weight-based)",
        route: "IV infusion, central",
        frequency: "Continuous",
        indications: "Catecholamine-sparing second agent in septic shock; vasoplegia",
        notes: "V1 receptor agonist, effective in acidosis; risk of digital and mesenteric ischaemia. Do not titrate upward beyond 0.03–0.04 units/min.",
        topicIds: ["vasoactive-agents", "sepsis"],
        paediatricDose: "0.0003–0.002 units/kg/min (0.18–1.2 milliunits/kg/min)",
        neonatalDose: "0.0001–0.001 units/kg/min",
        paediatricNotes: "Catecholamine-sparing in refractory shock and useful in pulmonary hypertension; watch for digital and splanchnic ischaemia and hyponatraemia.",
      },
      {
        drug: "Dobutamine",
        dose: "2.5–20 microgram/kg/min",
        route: "IV infusion",
        frequency: "Continuous",
        indications: "Low cardiac output with adequate filling and perfusion pressure; cardiogenic shock",
        notes: "β1 > β2 — may drop systemic vascular resistance and blood pressure; tachyarrhythmia common.",
        topicIds: ["vasoactive-agents", "circulatory-failure"],
        paediatricDose: "2.5–20 microgram/kg/min",
        neonatalDose: "5–20 microgram/kg/min",
        paediatricNotes: "Useful for low cardiac output with adequate blood pressure, e.g. myocarditis or post-cardiac surgery.",
      },
      {
        drug: "Milrinone",
        dose: "0.375–0.75 microgram/kg/min (loading dose usually omitted in shock)",
        route: "IV infusion",
        frequency: "Continuous",
        indications: "Inodilator for right ventricular failure, pulmonary hypertension, β-receptor downregulation",
        notes: "PDE-3 inhibitor; accumulates in renal impairment; vasodilatation often needs noradrenaline cover.",
        topicIds: ["vasoactive-agents", "pulmonary-hypertension"],
        paediatricDose: "0.25–0.75 microgram/kg/min; loading dose usually omitted",
        neonatalDose: "0.25–0.5 microgram/kg/min",
        paediatricNotes: "Inodilator of choice for low cardiac output syndrome after paediatric cardiac surgery; accumulates in renal impairment.",
      },
      {
        drug: "Metaraminol",
        dose: "0.5–2 mg bolus; 0.5–10 mg/h infusion",
        route: "IV",
        frequency: "PRN or continuous",
        indications: "Short-term peripheral vasopressor while central access is obtained",
        notes: "Predominantly α1; reflex bradycardia expected.",
        topicIds: ["vasoactive-agents"],
        paediatricDose: "Bolus 1–10 microgram/kg; infusion 0.1–0.5 microgram/kg/min",
        neonatalDose: "Not used",
        paediatricNotes: "Rarely used in children — adrenaline or noradrenaline is preferred.",
      },
      {
        drug: "Hydrocortisone",
        dose: "50 mg 6-hourly (or 200 mg/24 h infusion)",
        route: "IV",
        frequency: "6-hourly",
        indications: "Septic shock on ongoing vasopressor support; adrenal insufficiency",
        notes: "Monitor glucose; taper as vasopressors wean.",
        topicIds: ["sepsis", "steroids"],
        paediatricDose: "1–2 mg/kg 6-hourly (max 100 mg per dose)",
        neonatalDose: "1 mg/kg 8-hourly",
        paediatricNotes: "For catecholamine-resistant shock and suspected adrenal insufficiency (including known CAH, where doses are higher).",
      },
    ],
  },
  {
    id: "cardiac-rhythm",
    title: "Antiarrhythmics and cardiovascular drugs",
    blurb: "Correct potassium, magnesium, hypoxia, pain and sepsis before escalating drug therapy.",
    drugs: [
      {
        drug: "Amiodarone",
        dose: "300 mg over 20–60 min, then 900 mg over 23 h; arrest 300 mg bolus",
        route: "IV, central for infusion",
        frequency: "Loading then continuous",
        indications: "Atrial fibrillation with instability, ventricular tachyarrhythmia, shock-refractory VF/pVT",
        notes: "Phlebitis peripherally; causes hypotension when given fast; long-term thyroid, hepatic and pulmonary toxicity.",
        topicIds: ["arrhythmias-icu", "antiarrhythmics"],
        paediatricDose: "Shockable arrest 5 mg/kg IV/IO bolus (max 300 mg); perfusing tachycardia 5 mg/kg over 20 min, then 5–15 microgram/kg/min",
        neonatalDose: "5 mg/kg over 30 min; specialist advice",
        paediatricNotes: "Give slowly outside arrest — rapid injection causes hypotension; central access preferred.",
      },
      {
        drug: "Magnesium sulfate",
        dose: "2 g (8 mmol) over 10–20 min; 4 g then 1 g/h in eclampsia",
        route: "IV",
        frequency: "Bolus then infusion if indicated",
        indications: "Torsades de pointes, refractory AF, severe asthma, eclampsia, hypomagnesaemia",
        notes: "Monitor reflexes and respiratory rate; antidote is calcium gluconate 10 mL of 10%.",
        topicIds: ["arrhythmias-icu", "electrolytes", "obstetric-emergencies"],
        paediatricDose: "25–50 mg/kg (0.1–0.2 mmol/kg) over 20 min, max 2 g",
        neonatalDose: "25–50 mg/kg over 30 min",
        paediatricNotes: "For torsade, severe asthma and hypomagnesaemia; monitor for hypotension and areflexia.",
      },
      {
        drug: "Digoxin",
        dose: "Load 500 microgram then 250–500 microgram at 6 h; maintenance 62.5–250 microgram/day",
        route: "IV or oral",
        frequency: "Loading then daily",
        indications: "Rate control in AF, particularly with poor left ventricular function",
        notes: "Renally cleared, narrow therapeutic index; toxicity potentiated by hypokalaemia.",
        topicIds: ["arrhythmias-icu"],
        paediatricDose: "Loading 15 microgram/kg total in divided doses; maintenance 3–5 microgram/kg/day",
        neonatalDose: "Loading 20 microgram/kg divided; maintenance 4–5 microgram/kg/day",
        paediatricNotes: "Used for supraventricular tachycardia and heart failure in infants; narrow margin — check levels and potassium.",
      },
      {
        drug: "Esmolol",
        dose: "25–300 microgram/kg/min (load 0.5 mg/kg if tolerated)",
        route: "IV infusion",
        frequency: "Continuous",
        indications: "Rapid rate control, thyroid storm, aortic dissection, catecholamine excess",
        notes: "Very short acting (half-life ~9 min) — quickly reversible if hypotension occurs.",
        topicIds: ["arrhythmias-icu", "beta-blockers"],
        paediatricDose: "Load 100–500 microgram/kg over 1 min; infusion 25–200 microgram/kg/min",
        neonatalDose: "Specialist use only, 25–100 microgram/kg/min",
        paediatricNotes: "Short half-life makes it useful for coarctation repair and thyroid storm; stop if cardiac output falls.",
      },
      {
        drug: "Glyceryl trinitrate",
        dose: "0.5–10 mg/h",
        route: "IV infusion",
        frequency: "Continuous",
        indications: "Acute pulmonary oedema, hypertensive emergency, myocardial ischaemia",
        notes: "Tachyphylaxis within 24–48 h; avoid in severe aortic stenosis and right ventricular infarction.",
        topicIds: ["circulatory-failure", "vasoactive-agents"],
        paediatricDose: "0.5–5 microgram/kg/min",
        neonatalDose: "0.5–3 microgram/kg/min",
        paediatricNotes: "Titrate to blood pressure; tachyphylaxis develops within 24–48 h.",
      },
      {
        drug: "Labetalol",
        dose: "10–20 mg bolus, repeat to 200 mg; 20–160 mg/h infusion",
        route: "IV",
        frequency: "PRN or continuous",
        indications: "Hypertensive emergency, pre-eclampsia, controlled reduction after intracerebral haemorrhage",
        notes: "Combined α and β blockade; avoid in asthma and severe bradycardia.",
        topicIds: ["neuro-icu", "obstetric-emergencies"],
        paediatricDose: "Bolus 0.25–0.5 mg/kg; infusion 0.5–3 mg/kg/h",
        neonatalDose: "Avoid; specialist advice",
        paediatricNotes: "Used for hypertensive emergencies such as post-transplant or phaeochromocytoma after alpha blockade.",
      },
    ],
  },
  {
    id: "neuro",
    title: "Neurocritical care and seizures",
    blurb: "Treat status epilepticus by the clock; escalate at fixed time points rather than waiting for effect.",
    drugs: [
      {
        drug: "Lorazepam",
        dose: "4 mg (0.1 mg/kg), repeat once after 10 min",
        route: "IV",
        frequency: "Up to two doses",
        indications: "First-line status epilepticus",
        notes: "Midazolam 10 mg buccal or IM if no IV access.",
        topicIds: ["status-epilepticus", "neuro-icu"],
        paediatricDose: "100 microgram/kg IV (max 4 mg), repeat once after 10 min",
        neonatalDose: "100 microgram/kg IV",
        paediatricNotes: "First-line for convulsive status epilepticus in children per APLS; buccal midazolam 300–500 microgram/kg if no access.",
      },
      {
        drug: "Levetiracetam",
        dose: "60 mg/kg (max 4.5 g) over 10 min, then 500–1500 mg BD",
        route: "IV",
        frequency: "Load then 12-hourly",
        indications: "Second-line status epilepticus; seizure prophylaxis",
        notes: "Few interactions; reduce dose in renal impairment; behavioural effects.",
        topicIds: ["status-epilepticus", "anticonvulsants"],
        paediatricDose: "Load 40 mg/kg over 5 min (max 3 g); maintenance 20–30 mg/kg 12-hourly",
        neonatalDose: "Load 20–40 mg/kg; maintenance 10–20 mg/kg 12-hourly",
        paediatricNotes: "Second-line in paediatric status (as in the EcLiPSE trial) and widely used for neonatal seizures.",
      },
      {
        drug: "Phenytoin",
        dose: "20 mg/kg at ≤50 mg/min, then 100 mg 8-hourly",
        route: "IV with cardiac monitoring",
        frequency: "Load then 8-hourly",
        indications: "Second-line status epilepticus",
        notes: "Causes hypotension and arrhythmia if infused fast; zero-order kinetics; monitor levels and albumin.",
        topicIds: ["status-epilepticus", "anticonvulsants"],
        paediatricDose: "Load 20 mg/kg over 20 min with ECG monitoring; maintenance 2.5 mg/kg 12-hourly",
        neonatalDose: "Load 20 mg/kg over 20 min; maintenance 2.5 mg/kg 12-hourly",
        paediatricNotes: "Give into a large vein with saline — extravasation causes severe tissue injury; monitor for bradycardia while loading.",
      },
      {
        drug: "Hypertonic saline 2.7–5%",
        dose: "2–5 mL/kg of 2.7% (or 100 mL of 5%) over 10–20 min",
        route: "IV, central preferred",
        frequency: "Bolus, repeated to target",
        indications: "Raised intracranial pressure, cerebral oedema, symptomatic hyponatraemia (100 mL 2.7–3%)",
        notes: "Target sodium 145–155 mmol/L for ICP control; monitor sodium at least 4-hourly and limit correction to 8–10 mmol/L per 24 h in chronic hyponatraemia.",
        topicIds: ["icp-management", "neuro-icu", "electrolytes"],
        paediatricDose: "3% sodium chloride 3–5 mL/kg over 10–20 min for raised ICP; infusion 0.1–1 mL/kg/h",
        neonatalDose: "3 mL/kg of 3% over 20 min; specialist advice",
        paediatricNotes: "Target sodium 145–155 mmol/L for intracranial hypertension; central line preferred for concentrations above 3%.",
      },
      {
        drug: "Mannitol 20%",
        dose: "0.25–1 g/kg over 15–20 min",
        route: "IV",
        frequency: "PRN, 4–6 hourly maximum",
        indications: "Acutely raised intracranial pressure",
        notes: "Osmotic diuresis causes hypovolaemia and hypokalaemia; keep osmolality below ~320 mosmol/kg.",
        topicIds: ["icp-management", "traumatic-brain-injury"],
        paediatricDose: "0.25–0.5 g/kg (1.25–2.5 mL/kg of 20%) over 20 min",
        neonatalDose: "0.25 g/kg",
        paediatricNotes: "Check serum osmolality and volume status; ensure euvolaemia before repeat doses.",
      },
      {
        drug: "Nimodipine",
        dose: "60 mg 4-hourly enterally (or 1–2 mg/h IV)",
        route: "Enteral (preferred) or IV",
        frequency: "4-hourly for 21 days",
        indications: "Delayed cerebral ischaemia prophylaxis after aneurysmal subarachnoid haemorrhage",
        notes: "Hypotension is the dose-limiting effect; reduce or split doses rather than stopping.",
        topicIds: ["subarachnoid-haemorrhage", "neuro-icu"],
        paediatricDose: "Not routinely used; specialist advice for paediatric subarachnoid haemorrhage",
        neonatalDose: "Not used",
        paediatricNotes: "Aneurysmal subarachnoid haemorrhage is rare in children — discuss with the neurosurgical centre.",
      },
    ],
  },
  {
    id: "coagulation",
    title: "Haemostasis and anticoagulation",
    blurb: "Balance thrombotic and bleeding risk daily; use viscoelastic testing and laboratory clotting to guide product use.",
    drugs: [
      {
        drug: "Tranexamic acid",
        dose: "1 g over 10 min then 1 g over 8 h (trauma, within 3 h of injury)",
        route: "IV",
        frequency: "Load then infusion",
        indications: "Trauma haemorrhage, obstetric haemorrhage, surgical bleeding, hyperfibrinolysis",
        notes: "Give as early as possible; benefit lost and possible harm after 3 h in trauma.",
        topicIds: ["transfusion-coagulation", "trauma-resuscitation"],
        paediatricDose: "15 mg/kg (max 1 g) over 10 min, then 2 mg/kg/h for 8 h",
        neonatalDose: "15 mg/kg over 10 min; cardiac surgery per local protocol",
        paediatricNotes: "Give within 3 h of major trauma; also used for surgical and mucosal bleeding.",
      },
      {
        drug: "Enoxaparin",
        dose: "40 mg daily prophylaxis (20 mg if eGFR <30 mL/min); 1.5 mg/kg daily or 1 mg/kg BD treatment",
        route: "Subcutaneous",
        frequency: "Daily or 12-hourly",
        indications: "VTE prophylaxis and treatment",
        notes: "Accumulates in renal failure — use unfractionated heparin if eGFR is very low or bleeding risk high.",
        topicIds: ["transfusion-coagulation", "aki-rrt"],
        paediatricDose: "Prophylaxis 0.5 mg/kg 12-hourly; treatment 1 mg/kg 12-hourly",
        neonatalDose: "Under 2 months: prophylaxis 0.75 mg/kg and treatment 1.5 mg/kg 12-hourly",
        paediatricNotes: "Monitor anti-Xa in infants and renal impairment — dose requirements per kg are higher than in adults.",
      },
      {
        drug: "Unfractionated heparin",
        dose: "5000 units BD/TDS prophylaxis; treatment 75 units/kg bolus then 18 units/kg/h to APTR 1.5–2.5",
        route: "IV or subcutaneous",
        frequency: "Continuous or 8–12 hourly",
        indications: "Treatment-dose anticoagulation with renal failure or high bleeding risk; circuit anticoagulation",
        notes: "Reversed by protamine 1 mg per 100 units; monitor platelets for HIT.",
        topicIds: ["transfusion-coagulation", "aki-rrt"],
        paediatricDose: "Load 75 units/kg then 20 units/kg/h",
        neonatalDose: "Load 75 units/kg then 28 units/kg/h (under 1 year)",
        paediatricNotes: "Higher weight-based rates are needed because antithrombin levels are low in infancy; titrate to anti-Xa or APTT ratio.",
      },
      {
        drug: "Prothrombin complex concentrate",
        dose: "25–50 units/kg by indication and INR",
        route: "IV",
        frequency: "Single dose, repeat on results",
        indications: "Warfarin reversal with major bleeding (with vitamin K 5–10 mg IV); factor deficiency bleeding",
        notes: "Faster and lower volume than fresh frozen plasma; thrombotic risk.",
        topicIds: ["transfusion-coagulation"],
        paediatricDose: "25–50 units/kg guided by INR and weight",
        neonatalDose: "25–50 units/kg; haematology advice",
        paediatricNotes: "Reserve for life-threatening warfarin or vitamin K-dependent bleeding, with vitamin K 250–300 microgram/kg IV.",
      },
      {
        drug: "Andexanet alfa / idarucizumab",
        dose: "Idarucizumab 5 g IV; andexanet per protocol by dose and timing",
        route: "IV",
        frequency: "Single course",
        indications: "Life-threatening bleeding on dabigatran (idarucizumab) or apixaban/rivaroxaban (andexanet)",
        notes: "Availability is local and restricted; PCC is the fallback for factor Xa inhibitor bleeding.",
        topicIds: ["transfusion-coagulation"],
        paediatricDose: "Not licensed in children; discuss with haematology",
        neonatalDose: "Not used",
        paediatricNotes: "Direct oral anticoagulant use is rare in children; PCC and supportive care are usually used instead.",
      },
      {
        drug: "Regional citrate (CRRT)",
        dose: "Per circuit protocol, targeting post-filter ionised calcium 0.25–0.35 mmol/L",
        route: "Extracorporeal circuit",
        frequency: "Continuous",
        indications: "First-line circuit anticoagulation for continuous renal replacement therapy",
        notes: "Watch total:ionised calcium ratio >2.5 as a marker of citrate accumulation in liver failure or shock.",
        topicIds: ["aki-rrt"],
        paediatricDose: "Citrate dosed to circuit blood flow, target post-filter ionised calcium 0.25–0.35 mmol/L",
        neonatalDose: "Same targets; use paediatric circuits and low blood-flow settings",
        paediatricNotes: "Watch total:ionised calcium ratio above 2.5 as a marker of citrate accumulation, which occurs earlier in liver failure.",
      },
    ],
  },
  {
    id: "metabolic-gi",
    title: "Metabolic, endocrine and gastrointestinal",
    blurb: "Standard supportive prescriptions reviewed on every ward round alongside nutrition and glycaemic control.",
    drugs: [
      {
        drug: "Insulin (soluble)",
        dose: "Variable rate 0.5–6 units/h; DKA fixed rate 0.1 units/kg/h",
        route: "IV infusion",
        frequency: "Continuous",
        indications: "Hyperglycaemia (target ~6–10 mmol/L), DKA, hyperkalaemia (10 units with 25 g glucose)",
        notes: "Hourly glucose while titrating; continue insulin in DKA until ketones <0.6 mmol/L, adding glucose-containing fluid.",
        topicIds: ["diabetes-emergencies", "electrolytes"],
        paediatricDose: "DKA 0.05–0.1 units/kg/h (no bolus); hyperglycaemia 0.02–0.1 units/kg/h",
        neonatalDose: "0.01–0.05 units/kg/h",
        paediatricNotes: "In paediatric DKA start insulin 1–2 h after fluids, use 0.05 units/kg/h where cerebral oedema risk is high, and add glucose when blood glucose falls below 14 mmol/L.",
      },
      {
        drug: "Calcium gluconate 10%",
        dose: "10–30 mL over 5–10 min",
        route: "IV",
        frequency: "PRN, repeat on ECG response",
        indications: "Hyperkalaemic ECG change, hypocalcaemia, citrate toxicity, magnesium or calcium-channel blocker toxicity",
        notes: "Membrane stabilisation only — does not lower potassium; use central line for calcium chloride.",
        topicIds: ["electrolytes", "aki-rrt", "toxicology"],
        paediatricDose: "0.5 mL/kg (max 20 mL) slowly with cardiac monitoring",
        neonatalDose: "0.5 mL/kg over 5–10 min",
        paediatricNotes: "For hyperkalaemia with ECG change, hypocalcaemia and calcium-channel blocker toxicity; extravasation causes necrosis.",
      },
      {
        drug: "Potassium chloride",
        dose: "10 mmol/h peripherally; up to 20 mmol/h centrally with monitoring",
        route: "IV infusion",
        frequency: "Continuous until replaced",
        indications: "Hypokalaemia, especially with arrhythmia or DKA",
        notes: "Never give as a bolus; correct magnesium simultaneously or potassium will not stay corrected.",
        topicIds: ["electrolytes"],
        paediatricDose: "0.5 mmol/kg over 1–2 h, maximum 0.2 mmol/kg/h (central access for concentrated solutions)",
        neonatalDose: "0.5 mmol/kg over 2 h",
        paediatricNotes: "Never give undiluted; use ready-made bags and continuous ECG monitoring for rapid replacement.",
      },
      {
        drug: "Pantoprazole / omeprazole",
        dose: "40 mg daily (80 mg then 8 mg/h for major upper GI bleeding)",
        route: "IV or enteral",
        frequency: "Daily or infusion",
        indications: "Stress ulcer prophylaxis in the ventilated or coagulopathic patient; GI bleeding",
        notes: "Stop when risk factors resolve — associated with C. difficile and pneumonia.",
        topicIds: ["gi-bleeding", "nutrition-icu"],
        paediatricDose: "Omeprazole 0.5–1 mg/kg once daily IV (max 40 mg)",
        neonatalDose: "0.7 mg/kg once daily",
        paediatricNotes: "Stress-ulcer prophylaxis for the ventilated or coagulopathic child; review daily once feeding.",
      },
      {
        drug: "Terlipressin",
        dose: "1–2 mg 4–6 hourly",
        route: "IV",
        frequency: "4–6 hourly",
        indications: "Variceal haemorrhage; hepatorenal syndrome (with albumin)",
        notes: "Ischaemic complications and hyponatraemia; avoid in ischaemic heart disease.",
        topicIds: ["acute-liver-failure", "gi-bleeding"],
        paediatricDose: "5–20 microgram/kg 4-hourly; specialist use",
        neonatalDose: "Not used",
        paediatricNotes: "Occasionally used for variceal bleeding or hepatorenal syndrome under hepatology guidance; monitor sodium and perfusion.",
      },
      {
        drug: "N-acetylcysteine",
        dose: "100 mg/kg over 2 h, then 200 mg/kg over 10 h (SNAP two-bag regimen), continued as needed",
        route: "IV infusion",
        frequency: "Continuous course",
        indications: "Paracetamol overdose, non-paracetamol acute liver failure",
        notes: "Do not stop for non-IgE anaphylactoid reactions — slow or pause, treat, then restart.",
        topicIds: ["toxicology", "acute-liver-failure"],
        paediatricDose: "Weight-banded 21-h regimen (SNAP-style two-bag) with reduced fluid volumes below 20 kg",
        neonatalDose: "20 kg or less: use paediatric fluid volumes to avoid hyponatraemia",
        paediatricNotes: "Use the paediatric infusion volumes on TOXBASE — adult bag volumes cause fluid overload and hyponatraemia in small children.",
      },
      {
        drug: "Thiamine (Pabrinex)",
        dose: "One to two pairs of ampoules TDS for 3–5 days",
        route: "IV",
        frequency: "8-hourly",
        indications: "Suspected Wernicke's encephalopathy, alcohol dependence, refeeding risk",
        notes: "Give before carbohydrate load in malnutrition or alcohol excess.",
        topicIds: ["nutrition-icu", "toxicology"],
        paediatricDose: "Specialist use; 25–50 mg IV daily for suspected deficiency",
        neonatalDose: "Not routinely used",
        paediatricNotes: "Consider in malnutrition, prolonged parenteral nutrition and refeeding.",
      },
    ],
  },
  {
    id: "infection",
    title: "Antimicrobials and immunity",
    blurb: "Follow local antimicrobial policy and microbiology advice; take cultures first and review at 48–72 h for de-escalation.",
    drugs: [
      {
        drug: "Piperacillin–tazobactam",
        dose: "4.5 g 8-hourly (6-hourly or extended infusion in severe sepsis)",
        route: "IV",
        frequency: "6–8 hourly",
        indications: "Broad-spectrum empirical cover for severe sepsis and hospital-acquired infection",
        notes: "Adjust in renal impairment; augmented renal clearance may require higher or extended-infusion dosing.",
        topicIds: ["sepsis", "antimicrobials"],
        paediatricDose: "90 mg/kg 6–8 hourly (max 4.5 g per dose)",
        neonatalDose: "90 mg/kg 8-hourly; 12-hourly if under 32 weeks corrected",
        paediatricNotes: "Extended infusions are increasingly used in sepsis; adjust for renal function.",
      },
      {
        drug: "Meropenem",
        dose: "1 g 8-hourly (2 g 8-hourly for CNS infection)",
        route: "IV",
        frequency: "8-hourly",
        indications: "ESBL or resistant organisms, severe intra-abdominal or CNS infection",
        notes: "Lowers seizure threshold; reduces valproate levels.",
        topicIds: ["sepsis", "antimicrobials"],
        paediatricDose: "20–40 mg/kg 8-hourly (40 mg/kg for CNS infection, max 2 g)",
        neonatalDose: "20 mg/kg 12-hourly, rising to 8-hourly with maturity",
        paediatricNotes: "Preferred where ESBL or resistant Gram-negative organisms are likely; good CSF penetration.",
      },
      {
        drug: "Vancomycin",
        dose: "15–20 mg/kg loading, then level-guided (trough 15–20 mg/L for severe infection)",
        route: "IV infusion over ≥1 h",
        frequency: "Level-guided",
        indications: "MRSA, line infection, Gram-positive cover",
        notes: "Nephrotoxic — monitor levels and renal function; infusion reactions if given fast.",
        topicIds: ["sepsis", "antimicrobials"],
        paediatricDose: "15 mg/kg 6-hourly, then adjust to AUC or trough 10–15 mg/L",
        neonatalDose: "15 mg/kg every 8–24 h according to postmenstrual age and renal function",
        paediatricNotes: "Loading 20–25 mg/kg is used in severe sepsis; levels are essential in neonates.",
      },
      {
        drug: "Ceftriaxone",
        dose: "2 g daily (2 g 12-hourly for CNS infection)",
        route: "IV",
        frequency: "12–24 hourly",
        indications: "Community-acquired pneumonia, meningitis, meningococcal sepsis",
        notes: "Add amoxicillin for Listeria cover in the over-60s and immunosuppressed; give dexamethasone 10 mg 6-hourly in bacterial meningitis.",
        topicIds: ["cns-infection", "sepsis"],
        paediatricDose: "50–80 mg/kg once daily; 80–100 mg/kg daily for meningitis (max 4 g)",
        neonatalDose: "Avoid in jaundiced neonates and with calcium-containing infusions — use cefotaxime 50 mg/kg 6–12 hourly",
        paediatricNotes: "Ceftriaxone displaces bilirubin and precipitates with calcium in neonates.",
      },
      {
        drug: "Co-trimoxazole",
        dose: "PCP treatment 120 mg/kg/day in 2–4 divided doses; prophylaxis 480–960 mg daily",
        route: "IV or oral",
        frequency: "6–12 hourly (treatment); daily (prophylaxis)",
        indications: "Pneumocystis jirovecii pneumonia treatment and prophylaxis in immunosuppression",
        notes: "Add corticosteroid in PCP with PaO₂ <9.3 kPa; monitor potassium, renal function and marrow.",
        topicIds: ["haematology-icu", "immunology-intensivists"],
        paediatricDose: "Pneumocystis treatment 120 mg/kg/day in divided doses; prophylaxis 24 mg/kg once daily on 3 days a week",
        neonatalDose: "Prophylaxis from 4 weeks in HIV-exposed infants",
        paediatricNotes: "Monitor potassium, renal function and blood counts; give folinic acid on prolonged high-dose treatment.",
      },
      {
        drug: "Aciclovir",
        dose: "10 mg/kg 8-hourly (ideal body weight)",
        route: "IV",
        frequency: "8-hourly",
        indications: "Suspected herpes simplex encephalitis, severe varicella",
        notes: "Crystal nephropathy — hydrate well and adjust for renal function.",
        topicIds: ["cns-infection"],
        paediatricDose: "20 mg/kg 8-hourly (herpes encephalitis; 500 mg/m² in older children)",
        neonatalDose: "20 mg/kg 8-hourly for 14–21 days",
        paediatricNotes: "Ensure good hydration and dose-adjust in renal impairment; neonatal HSV needs 21 days of treatment for disseminated or CNS disease.",
      },
    ],
  },
  {
    id: "anticholinergic",
    title: "Anticholinergics and antimuscarinics",
    blurb: "Given for bradycardia, secretions and as part of neuromuscular reversal; every one of them causes tachycardia, dry mouth, urinary retention and — for the tertiary amines — delirium.",
    drugs: [
      {
        drug: "Atropine",
        dose: "300–600 microgram IV bolus, repeated to a maximum of 3 mg; 500 microgram–1 mg in cardiac arrest protocols where still used",
        route: "IV (IO, or 3 mg nebulised in organophosphate poisoning by protocol)",
        frequency: "PRN, repeat every 3–5 min",
        indications: "Haemodynamically significant bradycardia, vagal bradycardia during intubation or suctioning, organophosphate and nerve-agent poisoning, antisialagogue",
        notes: "Tertiary amine — crosses the blood–brain barrier and causes central anticholinergic delirium, especially in the elderly. Doses below 300 microgram can cause paradoxical bradycardia. In organophosphate poisoning the dose is titrated to drying of secretions, with doubling doses and often grams in total.",
        topicIds: ["arrhythmias", "toxicology", "autonomic-nervous-system"],
        paediatricDose: "20 microgram/kg IV (minimum 100 microgram, maximum 600 microgram per dose)",
        neonatalDose: "10–20 microgram/kg IV",
        paediatricNotes: "Bradycardia in a child is usually hypoxic — oxygenate and ventilate first; atropine is second-line except for vagal or drug-induced bradycardia.",
      },
      {
        drug: "Glycopyrronium (glycopyrrolate)",
        dose: "200–400 microgram IV for bradycardia; 200 microgram per 1 mg neostigmine for reversal; 200–400 microgram 4–6 hourly (or 0.6–1.2 mg/24 h by infusion) for secretions",
        route: "IV, IM or subcutaneous",
        frequency: "PRN or continuous subcutaneous infusion",
        indications: "Bradycardia, antisialagogue, drying of terminal secretions, co-administration with neostigmine",
        notes: "Quaternary amine — does not cross the blood–brain barrier, so no central anticholinergic delirium. Less tachycardia than atropine and a longer antisialagogue effect; renally cleared, so it accumulates in renal failure.",
        topicIds: ["autonomic-nervous-system", "end-of-life-care", "neuromuscular-blockers"],
        paediatricDose: "4–10 microgram/kg IV (max 200 microgram per dose); 10 microgram/kg with neostigmine",
        neonatalDose: "4–10 microgram/kg IV",
        paediatricNotes: "Preferred antisialagogue in children because it avoids central effects; watch for flushing and urinary retention.",
      },
      {
        drug: "Hyoscine butylbromide (Buscopan)",
        dose: "20 mg IV or subcutaneous, repeated up to 4–6 hourly; 60–120 mg/24 h by subcutaneous infusion",
        route: "IV, IM or subcutaneous",
        frequency: "4–6 hourly PRN or continuous",
        indications: "Excess respiratory secretions at the end of life, bowel colic and smooth-muscle spasm, bronchoscopy secretions",
        notes: "Quaternary — no central sedation or delirium, which makes it the first-line antisecretory drug in palliative care. Causes tachycardia and urinary retention; do not confuse with hyoscine hydrobromide.",
        topicIds: ["end-of-life-care", "gastrointestinal-physiology"],
        paediatricDose: "0.3–0.5 mg/kg (child) up to 20 mg per dose, 6–8 hourly",
        neonatalDose: "Not recommended",
        paediatricNotes: "Used for secretions and colic in palliative care under specialist advice only.",
      },
      {
        drug: "Hyoscine hydrobromide (scopolamine)",
        dose: "400 microgram subcutaneous 4–8 hourly, or 1 mg/72 h transdermal patch",
        route: "Subcutaneous or transdermal",
        frequency: "4–8 hourly PRN or patch every 72 h",
        indications: "Terminal secretions, antiemetic for motion-related and vestibular nausea",
        notes: "Tertiary amine — crosses into the CNS and is sedating, and a recognised cause of ICU and postoperative delirium in the elderly. Remove the patch and reconsider it in any new confusion.",
        topicIds: ["end-of-life-care", "antiemetics", "icu-sedation-delirium"],
        paediatricDose: "Patch quarter to half a patch by age under specialist advice; injection 10 microgram/kg 6-hourly",
        neonatalDose: "Avoid",
        paediatricNotes: "Avoid where possible — sedation, agitation and anticholinergic toxicity are common in children.",
      },
      {
        drug: "Ipratropium bromide",
        dose: "500 microgram nebulised 4–6 hourly (may be 4-hourly initially in acute severe asthma)",
        route: "Nebulised (or 2 puffs via a spacer / ventilator circuit)",
        frequency: "4–6 hourly",
        indications: "Acute severe asthma and COPD exacerbation alongside a β₂ agonist; bronchospasm in the ventilated patient",
        notes: "Quaternary and poorly absorbed, so systemic anticholinergic effects are limited; protect the eyes in glaucoma when using a face mask. Stop the regular 6-hourly dose once the acute phase settles.",
        topicIds: ["bronchospastic-failure", "respiratory-physiology"],
        paediatricDose: "Under 12 years 250 microgram nebulised, 12 years and over 500 microgram, up to 4–6 hourly",
        neonatalDose: "125–250 microgram nebulised 6–8 hourly under specialist advice",
        paediatricNotes: "Add to salbutamol in acute severe or life-threatening asthma; there is no benefit in continuing it once the child improves.",
      },
    ],
  },
];

export const icuDrugCount = icuDrugDoseGroups.reduce((n, g) => n + g.drugs.length, 0);
