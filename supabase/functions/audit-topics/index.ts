...
// For each topic:
//   1. Load the complete topic content from the build-time source corpus
//      (audit-corpus.json, generated by scripts/build-audit-corpus.ts) — no
//      page scraping, so collapsed panels and diagram labels are never missed
//   2. Retrieve citable evidence from Europe PMC / PubMed / Crossref
//   3. Ask the model to compare content against that evidence and emit findings,
//      each carrying a verbatim source quote plus a DOI or PMID
//   4. Audit diagrams from their verbatim labels in the corpus
//
          ...
          items: {
            ...
            properties: {
              ...
              source_quote: {
                type: "string",
                description:
                  "VERBATIM ≤400-char passage from one of the supplied evidence records that supports the finding. Empty string is permitted ONLY for thin/gap findings when no evidence was supplied; in that case `details` must name a specific guideline (title + year).",
              },
              evidence_doi: {
                type: "string",
                description:
                  "DOI of the evidence record that source_quote came from, exactly as shown (e.g. '10.1016/j.bjae.2020.02.005'). Empty string only when the record has no DOI.",
              },
              evidence_pmid: {
                type: "string",
                description:
                  "PubMed ID of the evidence record that source_quote came from. Empty string only when the record has no PMID.",
              },
...
type Stages = {
  ...
  // Source-corpus + literature pipeline
  corpus_ok?: boolean;
  corpus_source_files?: number;
  corpus_diagram_labels?: number;
...
async function auditTopic(
  ...
  // 1. Load the topic's content from the build-time source corpus.
  //
  // Previously this scraped the live page with Firecrawl. That was unreliable:
  // collapsed sections, tabbed panels and SVG diagram labels were often absent
  // from the rendered markdown, so the auditor judged content it could not see.
  // The corpus is generated from the app's own source files by
  // `scripts/build-audit-corpus.ts`, so it always contains the whole topic —
  // prose, worked examples, tables and every diagram label.
  const corpusEntry = getCorpusEntry(topic.id);

  if (!corpusEntry || corpusEntry.text.length < 500) {
    stages.corpus_ok = false;
    return {
      findings: [
        {
          severity: "major",
          category: "missing",
          summary: "Topic content missing from the audit corpus",
          details:
            `No source content was found for topic id "${topic.id}" in audit-corpus.json.\n\n` +
            `Regenerate it with \`bun run scripts/build-audit-corpus.ts\` and redeploy the ` +
            `audit function. This usually means the topic has no component under ` +
            `src/pages/topics/ or is missing from src/routes/topicRoutes.ts.`,
  ...
  const pageMarkdown = corpusEntry.text;
  // Diagrams are audited from their verbatim labels in the corpus; no page HTML
  // or screenshot is fetched any more.
  const pageHtml = "";
  const screenshot: string | undefined = undefined;
  stages.corpus_ok = true;
  stages.scrape_ok = true;
  stages.page_chars = pageMarkdown.length;
  stages.screenshot = false;
  stages.corpus_source_files = corpusEntry.source_files.length;
  stages.corpus_diagram_labels = corpusEntry.diagram_labels.length;
  console.log(
    `[audit-topics] corpus ${topic.id}: ${pageMarkdown.length} chars, ` +
      `${corpusEntry.source_files.length} source files, ` +
      `${corpusEntry.diagram_labels.length} diagram labels, ` +
      `${corpusEntry.references.length} declared references`,
  );

  // 2. Retrieve evidence from the research databases (Europe PMC → PubMed →
  // Crossref). These return structured records with a DOI/PMID and a verbatim
  // abstract, so every finding can be traced to a citable primary source
  // instead of to whatever a search-engine snippet happened to contain.
  let literature: LiteratureRecord[] = [];
  let recentGuidance: LiteratureRecord[] = [];
  if (hasBudget(30_000)) {
    try {
      [literature, recentGuidance] = await Promise.all([
        searchTopicEvidence(topic.title, {
          limit: 6,
          timeoutMs: 14_000,
          context: `${corpusEntry.description} ${corpusEntry.section_label}`,
        }),
        searchRecentGuidance(topic.title, { limit: 3, timeoutMs: 14_000 }),
      ]);
    } catch (e) {
      stages.literature_error = (e as Error).message;
      console.warn(`[audit-topics] literature lookup failed for ${topic.id}: ${(e as Error).message}`);
    }
  }

  const seenEvidence = new Set<string>();
  const evidence = [...literature, ...recentGuidance].filter((r) => {
    const key = r.doi ?? r.pmid ?? r.title.toLowerCase();
    if (seenEvidence.has(key)) return false;
    seenEvidence.add(key);
    return true;
  });

  const refs = evidence.map((r) => ({
    title: `${r.title}${r.journal ? ` — ${r.journal}` : ""}${r.year ? ` (${r.year})` : ""}`,
    url: r.url,
    excerpt: r.excerpt.slice(0, 3500),
    doi: r.doi,
    pmid: r.pmid,
    journal: r.journal,
    year: r.year,
  ...
  stages.evidence_reputable = evidence.filter((r) => isReputableJournal(r.journal)).length;
  stages.evidence_retracted = evidence.filter((r) => r.isRetracted).length;

  // The topic's own declared citations — the model is told to prefer evidence
  // that either confirms or contradicts these.
  const declaredRefsBlock = corpusEntry.references.length === 0
    ? "(this topic declares no citations in src/data/references.ts)"
    : corpusEntry.references
        .map((r, i) => `  ${i + 1}. [${r.label}] ${r.citation}${r.doi ? ` doi:${r.doi}` : ""}${r.pmid ? ` PMID:${r.pmid}` : ""}`)
        .join("\n");
    ...
    : refs
        .map((r, i) =>
          [
            ...
            `${r.doi ? `doi: ${r.doi}` : ""}${r.pmid ? `${r.doi ? "  " : ""}PMID: ${r.pmid}` : ""}`.trim(),
            r.url,
            r.isRetracted ? "⚠️ THIS RECORD IS RETRACTED — do not use it as evidence." : "",
            r.excerpt,
          ]
            .filter(Boolean)
            .join("\n"),
  ...
  const evidenceRules = [
    "=== EVIDENCE RULES ===",
    "The evidence records below come from Europe PMC / PubMed / Crossref and each carries a DOI or PMID.",
    "Every accuracy, outdated or update finding MUST quote one of them verbatim in source_quote AND put that record's",
    "identifier in evidence_doi (preferred) or evidence_pmid. No verbatim quote and identifier ⇒ no finding.",
    "Never cite a record flagged as retracted.",
    "",
    "=== CITATIONS THIS TOPIC ALREADY DECLARES ===",
    declaredRefsBlock,
  ].join("\n");

  const refsBlock = `${evidenceRules}\n\n${refsBody}`;
  ...
  // Resolve each finding's cited identifier back to the literature record it
  // came from, so the stored finding carries journal / year / open-access status
  // and the dashboard can deep-link to DOI and PubMed.
  const cleanId = (v: unknown) => {
    const s = String(v ?? "").trim().replace(/^https?:\/\/(dx\.)?doi\.org\//i, "");
    return s && s !== "-" ? s : null;
  };
  for (const f of all) {
    const doi = cleanId(f.evidence_doi)?.toLowerCase() ?? null;
    const pmid = cleanId(f.evidence_pmid);
    f.evidence_doi = doi;
    f.evidence_pmid = pmid && /^\d+$/.test(pmid) ? pmid : null;
    const match = refs.find(
      (r) => (doi && r.doi === doi) || (f.evidence_pmid && r.pmid === f.evidence_pmid),
    );
    f.evidence_journal = match?.journal ?? null;
    f.evidence_year = match?.year ?? null;
    f.evidence_is_open_access = match ? match.isOpenAccess : null;
    // Drop identifiers the model invented — they did not come from the evidence
    // we supplied, so they cannot be trusted as citations.
    if (!match) {
      f.evidence_doi = null;
      f.evidence_pmid = null;
    }
    if (match && (!f.sources || f.sources.length === 0)) {
      f.sources = [{ title: match.title, url: match.url }];
    }
  }
...
async function runBatch(jobId: string) {
  ...
  // Paused-state guard: a 402/403 denial parks the audit, and every entry point
  // (manual start, self-chain, scheduled run) checks that before doing work.
  const { data: jobState } = await supa
    .from("audit_job_state")
    .select("paused, paused_reason")
    .eq("id", "content-audit")
    .maybeSingle();
  if (jobState?.paused) {
    console.warn(`[audit-topics] audit is paused: ${jobState.paused_reason ?? "no reason recorded"}`);
    await supa.from("topic_audit_jobs").update({
      status: "completed_with_errors",
      current_topic: null,
      last_error: `Audit paused: ${jobState.paused_reason ?? "resolve the pause reason, then resume"}`,
  ...
  // AI-gateway circuit breaker: 402 (out of AI credits) / 403 (blocked by
  // workspace policy) are terminal, so halt the whole sweep instead of
  // spending another 140 invocations on the same denial.
  ...
  for (let i = cursor; i < end; i++) {
      ...
      try {
        ...
        const aiErrors = [
          stages.text_error,
          stages.coverage_error,
          stages.freshness_error,
          stages.diagram_error,
          stages.diagram_label_error,
        ].filter(Boolean).join(" | ");
        if (/AI gateway 40[23]\b/.test(aiErrors)) {
          aiBlocked = aiErrors.slice(0, 400);
        } else if (/AI gateway 429\b/.test(aiErrors)) {
        ...
        if (findings.length > 0) {
          ...
          const allRows = findings.map((f) => ({
            ...
            // Citable evidence behind the finding — resolved back to the
            // literature record so the dashboard can deep-link to DOI/PubMed.
            evidence_quote: f.source_quote && String(f.source_quote).trim()
              ? String(f.source_quote).trim().slice(0, 1000)
              : null,
            evidence_doi: f.evidence_doi ?? null,
            evidence_pmid: f.evidence_pmid ?? null,
            evidence_journal: f.evidence_journal ?? null,
            evidence_year: f.evidence_year ?? null,
            evidence_is_open_access: f.evidence_is_open_access ?? null,
      ...
      if (aiBlocked || consecutiveRateLimits >= 4) {
        const reason = aiBlocked
          ? `Audit paused — the AI gateway refused the request: ${aiBlocked}. ` +
            `Top up AI credits (402) or re-enable Lovable AI / raise the workspace limit (403), then re-run.`
          : `Audit paused after ${consecutiveRateLimits} consecutive AI rate limits (429). ` +
            `Rate limits are transient — re-run the audit later.`;
        console.error(`[audit-topics] halting job ${jobId}: ${reason}`);
        await supa.from("topic_audit_jobs").update({
          status: "completed_with_errors",
          current_topic: null,
          last_error: reason,
        ...
        await supa.from("audit_job_state").update({
          paused: Boolean(aiBlocked),
...
/**
 * Topics to audit, taken from the source corpus. This is the authoritative list
 * — it is generated from the app's own route table, so it can never drift from
 * what is actually shipped, and it needs no network call. The sitemap crawl below
 * remains only as a fallback for the (unexpected) case of an empty corpus.
 */
function topicsFromCorpus(): TopicRef[] {
  return Array.from(CORPUS.values()).map((e) => ({
    id: e.topic_id,
    title: e.topic_title,
    section: e.section,
...
async function discoverTopics(): Promise<TopicRef[]> {
  const fromCorpus = topicsFromCorpus();
  if (fromCorpus.length > 0) {
    console.log(
      `[audit-topics] ${fromCorpus.length} topics from source corpus (generated ${CORPUS_GENERATED_AT ?? "unknown"})`,
    );
    return fromCorpus;
  }
  console.warn("[audit-topics] corpus empty — falling back to sitemap discovery");
  return await discoverTopicsFromSitemaps();
}
