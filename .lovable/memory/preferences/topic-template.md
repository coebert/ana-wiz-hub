...
4. **Worked Examples** — optional. Clinical vignettes + numerical calculations where relevant. Skip on topics where it doesn't fit (e.g. ethics-only). Use `WorkedExample[]` with `{title, scenario, working, answer, cites?}`.
5. **Key Learning Points** — final summary bullets via `keyPoints` prop. Each entry is either a string OR `{ text, cites? }`.
6. **Quiz → References → See Also → Completion toggle** — auto-rendered by template.

**Inline citations (`cites` field):** Optional but encouraged. Reference labels (must exist in `topicReferences[topicId]`) listed in the `cites` array on a worked example or key point are rendered as numeric superscripts (¹²) next to the title/text. Numbers match the ordered master list at the bottom of the page; clicking scrolls to the matching `#ref-<slug>` in the auto-rendered `<ReferencesList>`. Implemented via `<Cite topicId labels={[...]} />`.

**In-page references panel (`sectionSources` prop):** For `workedExamples` and `keyPoints` the panel is now AUTO-GENERATED from each entry's `cites` field — TopicTemplate collects unique labels, sorts them by position in `topicReferences[topicId]`, and renders a `<SectionReferences>` block beneath the section. Pass `sectionSources.workedExamples` / `sectionSources.keyPoints` only to OVERRIDE the auto-derived list. `objectives` and `diagrams` still need explicit `sectionSources` because their citations live inline in JSX/strings and can't be harvested. Each panel has a "Jump to" anchor (`#objectives`, `#diagrams`, `#worked-examples`, `#key-points`); per-item anchors also exist (`#objective-N`, `#example-N`, `#keypoint-N`). Unknown labels are ignored at runtime with a dev-only warning.
...
**Why:** Consistent learner experience across FRCA/FFICM/EDIC; enforces curriculum-aligned scaffolding (objectives → content → application → recall); surfaces evidence base inline with bidirectional jump links and numeric superscript citations; lets learners filter the page down to only the exam they're revising for.
